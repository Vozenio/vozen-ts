...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // vozen's own remote-access settings API (RemoteAccessSettingsSection,
      // connectManager.ts) — not part of bb's /api contract, so it needs its
      // own proxy entry or dev-mode fetches fall through to the SPA shell.
      "/vozen": {
        target: viteDevConfig.serverHttpOrigin,
        changeOrigin: true,
        xfwd: true,
