...
describe("useThreadTimeline optimistic row carry-forward", () => {
  function makeUserRow(id: string, text: string): TimelineUserConversationRow {
    return {
      id,
      kind: "conversation",
      role: "user",
  ...
  it("keeps an unresolved optimistic row across a refetch that omits it", async () => {
    mockMatchMedia([]);
    const optimisticRow = makeUserRow(
      `${OPTIMISTIC_TIMELINE_ROW_ID_PREFIX}a1`,
      "hello",
    );
    const { queryClient, wrapper } = createQueryClientTestHarness();
    queryClient.setQueryData(
      threadTimelineQueryKey("thread-1"),
      makeTimelineResponse([optimisticRow], 5),
      { updatedAt: 1 },
    );

    // An unrelated realtime event refetches the timeline while the server has
    // not yet projected the send into a row of its own.
    vi.mocked(sdk.threads.timeline).mockResolvedValueOnce(
    ...
    await waitFor(() => {
      ...
      expect(cached?.rows.map((row) => row.id)).toEqual([optimisticRow.id]);
    });
  });

  it("drops the optimistic row once a matching authoritative row lands, without duplicating it", async () => {
    mockMatchMedia([]);
    const optimisticRow = makeUserRow(
      `${OPTIMISTIC_TIMELINE_ROW_ID_PREFIX}a1`,
      "hello",
    ...
    const authoritativeRow = makeUserRow("thread-1:user:5", "hello");
    vi.mocked(sdk.threads.timeline).mockResolvedValueOnce(
      makeTimelineResponse([authoritativeRow], 6),
    );
    ...
    await waitFor(() => {
      ...
      expect(cached?.rows.map((row) => row.id)).toEqual([
        authoritativeRow.id,
      ]);
    });
  });
