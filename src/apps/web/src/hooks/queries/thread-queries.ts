...
/**
 * Whether an optimistic row's send has already landed in `rows` as a real,
 * server-authored row. There is no clientMessageId to match on, so this falls
 * back to a content match (same thread, same user-authored text) — good
 * enough because at most one optimistic row per distinct text is ever live.
 *
 * ponytail: content match, not a correlation id; upgrade to matching on a
 * clientMessageId if two identical-text sends in flight at once becomes a
 * real complaint.
 */
function isOptimisticRowResolved(
  optimisticRow: TimelineRow,
  rows: readonly TimelineRow[],
): boolean {
  if (optimisticRow.kind !== "conversation" || optimisticRow.role !== "user") {
    return true;
  }
  return rows.some(
    (row) =>
      ...
      row.role === "user" &&
      row.threadId === optimisticRow.threadId &&
      row.text === optimisticRow.text,
  );
}

/**
 * Carries forward any optimistic row from the previous window that a fresh
 * server response neither includes nor has superseded. Optimistic rows are
 * client-only (the server never learns their id), so a delta's `rowOrder` or
 * a full re-fetch naturally omits them — without this they vanish from the
 * timeline the moment any unrelated realtime event triggers a refetch, until
 * the authoritative row for the same send eventually lands.
 */
function withUnresolvedOptimisticRows(
  response: ThreadTimelineResponse,
  previousRows: readonly TimelineRow[] | undefined,
): ThreadTimelineResponse {
  if (!previousRows) {
    return response;
  }
  const presentIds = new Set(response.rows.map((row) => row.id));
  const survivors = previousRows.filter(
    (row) =>
      isOptimisticTimelineRowId(row.id) &&
      !presentIds.has(row.id) &&
      !isOptimisticRowResolved(row, response.rows),
  );
  if (survivors.length === 0) {
    return response;
  }
  return { ...response, rows: [...response.rows, ...survivors] };
}
 ...
 * can reconstruct our exact window). Either way, an optimistic row not yet
 * resolved by the response is carried forward — see
 * {@link withUnresolvedOptimisticRows}.
 */
async function mergeThreadTimelineDelta(
  ...
  if (response.delta === undefined) {
    return withUnresolvedOptimisticRows(response, previous?.rows);
  }
  const merged = previous
    ? applyTimelineDelta(previous.rows, response.delta)
    : null;
  if (merged !== null) {
    return withUnresolvedOptimisticRows(
      { ...response, rows: merged, delta: undefined },
      previous?.rows,
    );
  }
  const full = await fetchFull();
  return withUnresolvedOptimisticRows(full, previous?.rows);
}
