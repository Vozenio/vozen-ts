...
export function WorktreePicker({
  ...
  const triggerLabel =
    ...
    t("ui.worktreePicker.pickAWorktree");
  return (
    <DropdownMenu defaultOpen={defaultOpen} modal={modal}>
      <DropdownMenuTrigger asChild disabled={disabled}>
        <Button
          ...
          variant="ghost"
          size="sm"
          aria-label={t("ui.worktreePicker.worktree")}
          disabled={disabled}
      ...
      <DropdownMenuContent
        ...
        mobileTitle={t("ui.worktreePicker.worktree")}
      >
        <DropdownMenuLabel>
          {t("ui.worktreePicker.reuseExistingWorktree")}
        </DropdownMenuLabel>
        {options.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">
            {t("ui.worktreePicker.noWorktreesYet")}
          </div>
        ) : (
          options.map((option) => (
            <WorktreeMenuItem
...
function WorktreeMenuItem({
  ...
  const branchIcon = getEnvironmentWorkspaceLabelIconName("managed-worktree");
  const label = option.name ?? option.branchName ?? t("ui.worktreePicker.worktree");
        ...
        <span className="flex flex-col gap-0.5 pl-6 text-xs text-muted-foreground">
          ...
          {additionalCount > 0 ? (
            <span className="text-muted-foreground">
              {t("ui.worktreePicker.moreCount", { count: additionalCount })}
            </span>
          ) : null}
        </span>
      ) : null}
