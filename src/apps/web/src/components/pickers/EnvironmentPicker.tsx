...
export function EnvironmentPickerUI({
  ...
  const localLabel = isLocal
    ? t("ui.environmentPicker.workLocally")
    : t("ui.environmentPicker.workRemotely");
  ...
  const hostUnavailableReason = !host
    ? t("ui.environmentPicker.noHostConnected")
    : !hostConnected
      ? t("ui.environmentPicker.hostOffline")
      : null;
  const workspaceDisabledReason = hasSource
    ? null
    : t("ui.environmentPicker.projectSourceUnavailable");
  ...
  const reuseDisabledReason = reuseDisabled
    ? t("ui.environmentPicker.noWorktreesYet")
    : null;
  ...
  const selected = useMemo((): SelectedEnvironment => {
    ...
    if (hostUnavailableReason !== null) {
      return {
        ...
        compactModeLabel: host
          ? t("ui.environmentPicker.offline")
          : t("ui.environmentPicker.noHost"),
        icon: "AlertTriangle" as const,
      };
    }
    if (!parsed) {
      return {
        modeLabel: t("ui.environmentPicker.environment"),
        compactModeLabel: t("ui.environmentPicker.env"),
        icon: "Laptop" as const,
      };
    }
    if (parsed.type === "reuse") {
      return {
        modeLabel: t("ui.environmentPicker.reuseWorktree"),
        compactModeLabel: t("ui.environmentPicker.reuse"),
        icon: getEnvironmentWorkspaceLabelIconName("managed-worktree"),
      };
    }
    const modeLabel =
      parsed.mode === "worktree"
        ? t("ui.environmentPicker.newWorktree")
        : localLabel;
    const compactModeLabel =
      parsed.mode === "worktree"
        ? t("ui.environmentPicker.worktree")
        : isLocal
          ? t("ui.environmentPicker.local")
          : t("ui.environmentPicker.remote");
    ...
    <DropdownMenu defaultOpen={defaultOpen} modal={modal}>
      <DropdownMenuTrigger asChild disabled={disabled}>
        <Button
          ...
          variant="ghost"
          size="sm"
          aria-label={t("ui.environmentPicker.environment")}
          disabled={disabled}
      ...
      <DropdownMenuContent
        ...
        mobileTitle={t("ui.environmentPicker.environment")}
      >
    ...
    <DropdownMenuGroup>
      ...
      {hostUnavailableReason !== null ? (
        ...
        <>
          ...
          />
          <EnvironmentMenuItem
            label={t("ui.environmentPicker.newWorktree")}
          ...
          />
          <EnvironmentMenuItem
            label={t("ui.environmentPicker.existingWorktree")}
  ...
  return (
    <DropdownMenuGroup>
        ...
        <span className="flex items-center gap-1.5">
          ...
          {isThisMachine ? (
            <span className={MACHINE_BADGE_CLASS_NAME}>
              {t("ui.environmentPicker.thisMachine")}
            </span>
          ) : null}
          {formatHostUpdateStatus(host) !== null ? (
            ...
            <span className="ml-auto shrink-0 pl-2 text-2xs">
              {t("ui.environmentPicker.lastSeen")}{" "}
      ...
      {source ? (
        <>
          <EnvironmentMenuItem
            label={
              isThisMachine
                ? t("ui.environmentPicker.workLocally")
                : t("ui.environmentPicker.workInCheckout")
            }
            hint={source.path}
            icon={getEnvironmentWorkspaceLabelIconName("other")}
        ...
        <EnvironmentMenuItem
          label={t("ui.environmentPicker.setUpOn", { name: host.name })}
          icon="Plus"
          selected={false}
        ...
        <DropdownMenuItem
          ...
          className="whitespace-normal break-words text-xs text-muted-foreground"
        >
          {t("ui.environmentPicker.notSetUp")}
        </DropdownMenuItem>
      )}
    </DropdownMenuGroup>
  );
