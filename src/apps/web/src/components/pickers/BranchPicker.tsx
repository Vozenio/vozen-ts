...
function getCreateNewBranchLabel(t: TFn): string {
  return t("ui.branchPicker.createNew");
}
const EMPTY_BRANCH_OPTIONS: readonly string[] = [];
function getBranchLabelPrefixes(t: TFn): readonly string[] {
  return [
    t("ui.branchPicker.startFrom"),
    t("ui.branchPicker.currentColon"),
    t("ui.branchPicker.checkoutColon"),
    t("ui.branchPicker.newBranchFrom"),
    t("ui.branchPicker.branchFrom"),
  ];
}
function getCurrentParenthesesLabelPrefix(t: TFn): string {
  return `${t("ui.branchPicker.current")} (`;
}
function getDetachedLabelPrefix(t: TFn): string {
  return t("ui.branchPicker.detached");
}
...
function getGenericBranchMenuCopy(t: TFn): BranchPickerMenuCopy {
  return {
    title: null,
    currentSectionLabel: t("ui.branchPicker.current"),
    optionsSectionLabel: t("ui.branchPicker.branches"),
    optionsUnavailableFallback: t("ui.branchPicker.genericUnavailable"),
  };
}

function getCheckoutBranchMenuCopy(t: TFn): BranchPickerMenuCopy {
  return {
    title: t("ui.branchPicker.startFrom"),
    currentSectionLabel: null,
    optionsSectionLabel: t("ui.branchPicker.checkoutColon"),
    optionsUnavailableFallback: t("ui.branchPicker.checkoutUnavailable"),
  };
}

function getBaseBranchMenuCopy(t: TFn): BranchPickerMenuCopy {
  return {
    title: t("ui.branchPicker.branchFrom"),
    currentSectionLabel: null,
    optionsSectionLabel: null,
    optionsUnavailableFallback: t("ui.branchPicker.baseUnavailable"),
  };
}
...
function formatCreateBranchTriggerLabel(
  branch: string | null,
  t: TFn,
): string {
  return branch === null
    ? getCreateNewBranchLabel(t)
    : `${t("ui.branchPicker.newBranchFrom")} ${branch}`;
}

function splitBranchLabel(label: string, t: TFn): BranchLabelParts {
  const currentParenthesesLabelPrefix = getCurrentParenthesesLabelPrefix(t);
  if (
    label.startsWith(currentParenthesesLabelPrefix) &&
    label.endsWith(")")
  ) {
    return {
      kind: "parenthetical",
      prefix: t("ui.branchPicker.current"),
      value: label.slice(currentParenthesesLabelPrefix.length, -1),
    };
  }

  for (const prefix of getBranchLabelPrefixes(t)) {
  ...
  const detachedLabelPrefix = getDetachedLabelPrefix(t);
  const detachedPrefixWithSpace = `${detachedLabelPrefix} `;
  if (label.startsWith(detachedPrefixWithSpace)) {
    return {
      kind: "prefixed",
      prefix: detachedLabelPrefix,
...
function BranchPickerText({
  ...
  const compactAffixProps = compactAffixesInPromptbox
    ...
    : {};
  if (label === getCreateNewBranchLabel(t)) {
    return (
      <span className={cn("flex min-w-0 items-baseline gap-1", className)}>
        ...
        >
          {t("ui.branchPicker.newWord")}
        </span>
        <span {...compactAffixProps} className="shrink-0 text-muted-foreground">
          {t("ui.branchPicker.branchWord")}
        </span>
      </span>
    );
  }

  const parts = splitBranchLabel(label, t);
  if (parts.kind === "plain") {
    return (
      <span
        className={cn(
    ...
    <div className="shrink-0 border-b border-border p-1.5">
      <div className="relative">
        ...
        <Input
          ...
          placeholder={t("ui.branchPicker.searchPlaceholder")}
...
function getBranchPickerMenuCopy(
  ...
  switch (menuKind) {
    case "checkout":
      return getCheckoutBranchMenuCopy(t);
    case "base":
      return getBaseBranchMenuCopy(t);
    case undefined:
      return getGenericBranchMenuCopy(t);
  }
}
...
export function BranchPicker({
  ...
  const menuCopy = getBranchPickerMenuCopy(menuKind, t);
  ...
  const unresolvedTriggerLabel = loading
    ? t("ui.branchPicker.loadingBranches")
    : (placeholder ?? t("ui.branchPicker.selectBranch"));
  const triggerLabel =
    triggerLabelOverride ??
    (isCreatingNew
      ? formatCreateBranchTriggerLabel(value, t)
  ...
  const createDisabledDescription = formatUnavailableDescription({
    ...
    fallback: t("ui.branchPicker.newBranchesUnavailable"),
  });
    ...
    <Popover modal={modal} open={open} onOpenChange={updateOpen}>
      <PopoverTrigger asChild disabled={disabled}>
        <Button
          ...
          size="sm"
          disabled={disabled}
          aria-label={t("ui.branchPicker.branch")}
          className={cn(
            LIST_HOVER_TRANSITION,
            variant === "default" &&
      ...
      <PopoverContent
        ...
        sideOffset={6}
        collisionPadding={16}
        mobileTitle={menuCopy.title ?? t("ui.branchPicker.branch")}
        className={cn(
            ...
            {isCheckoutMenu ? (
              <>
                ...
                {showCreateItem && onCreate ? (
                  <BranchPickerRowButton
                    icon="Plus"
                    label={getCreateNewBranchLabel(t)}
                    title={createDisabledTitle ?? getCreateNewBranchLabel(t)}
                ...
                <BranchPickerRowButton
                  icon="GitMerge"
                  label={t("ui.branchPicker.checkout")}
                  title={
                    optionDisabledTitle ?? t("ui.branchPicker.checkoutExisting")
                  }
                          ...
                          <>
                            ...
                            {filteredCheckoutTargetOptions.length === 0 ? (
                              <p className="px-2 py-3 text-center text-xs text-muted-foreground">
                                {loading
                                  ? t("ui.branchPicker.loadingBranches")
                                  : t("ui.branchPicker.noLocalBranches")}
                              </p>
                            ) : null}
                          </>
                        ) : (
                          <>
                            ...
                            {filteredBranchOptions.length === 0 ? (
                              <p className="px-2 py-3 text-center text-xs text-muted-foreground">
                                {loading
                                  ? t("ui.branchPicker.loadingBranches")
                                  : t("ui.branchPicker.noBranchesFound")}
                              </p>
                            ) : null}
                          </>
                        )}
