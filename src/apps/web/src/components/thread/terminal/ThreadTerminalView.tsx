...
function writeTerminalSessionStatusNotice({
  ...
  switch (session.status) {
    case "disconnected":
      ...
      writeTerminalStatus({
        terminal,
        text: t("thread.terminal.disconnected", "Terminal disconnected"),
      });
      return;
    case "exited":
      ...
      writeTerminalStatus({
        ...
        text:
          session.exitCode === null
            ? t("thread.terminal.exited", "Terminal exited")
            : t(
                "thread.terminal.exitedWithCode",
  ...
  switch (message.type) {
    ...
    case "error":
      writeTerminalStatus({
        terminal,
        text: t(
          "thread.terminal.connectionError",
      ...
      const activeTransport = new TerminalWebSocketTransport({
        onConnectionState: (state) => {
          ...
          if (state === "open") {
            if (hasOpened && reconnectNoticeVisible) {
              writeTerminalStatus({
                terminal: activeTerminal,
                text: tRef.current("thread.terminal.reconnected", "Terminal reconnected"),
              });
            }
            hasOpened = true;
        ...
        onInputOverflow: (maxBytes) => {
          writeTerminalStatus({
            terminal: activeTerminal,
            text: tRef.current(
              "thread.terminal.inputQueueFull",
        ...
        onInvalidMessage: () => {
          writeTerminalStatus({
            terminal: activeTerminal,
            text: tRef.current(
              "thread.terminal.invalidServerMessage",
              "Terminal received an invalid server message",
            ),
          });
        },
        onMessage: (message) => {
