...
function getInactiveTerminalContent({
  ...
  status,
  t,
}: GetInactiveTerminalContentArgs & { t: TFunction }): InactiveTerminalContent {
  switch (status) {
    case "disconnected":
      return {
        canStartReplacement: canCreateTerminal,
        title: t("thread.terminal.disconnected", "Terminal disconnected"),
      };
    case "exited":
      return {
        canStartReplacement: false,
        title: t("thread.terminal.exited", "Terminal exited"),
      };
    case "starting":
      return {
        canStartReplacement: false,
        title: t("thread.terminal.starting", "Terminal starting"),
      };
    case "running":
      return {
        canStartReplacement: false,
        title: t("thread.terminal.running", "Terminal running"),
      };
  }
}
  ...
  if (controller.hasTerminalQueryError) {
    return (
      <div className="flex h-full items-center justify-center px-4 text-center text-sm text-destructive-text">
        {t("thread.terminal.loadFailed", "Failed to load terminals.")}
      </div>
    );
  }
        ...
        <div className="flex max-w-md flex-col items-center gap-3">
          ...
          {inactiveContent.canStartReplacement ? (
            ...
            >
              ...
              {t("thread.terminal.startNew", "Start new terminal")}
            </Button>
          ) : null}
        </div>
      </div>
