...
function toAttachmentPreviewLabel(
  ...
  if (!attachments) return t("thread.toc.attachmentMessage", "Message");
  const imageCount = attachments.webImages + attachments.localImages;
  const totalCount = imageCount + attachments.localFiles;
  if (totalCount === 0) return t("thread.toc.attachmentMessage", "Message");
  if (totalCount === 1) {
    return imageCount === 1
      ? t("thread.toc.attachmentImage", "Image attachment")
      : t("thread.toc.attachmentFile", "File attachment");
  }
  return t("thread.toc.attachmentCount", `${totalCount} attachments`, {
    count: totalCount,
  });
}
...
function toTocLabel({
  ...
  return textLabel || toAttachmentPreviewLabel(attachments, t);
}
...
function toAttachmentSummaryLabel(
  ...
  if (!summary) return t("thread.toc.attachmentMessage", "Message");
  const totalCount = summary.imageCount + summary.fileCount;
  if (totalCount === 0) return t("thread.toc.attachmentMessage", "Message");
  if (totalCount === 1) {
    return summary.imageCount === 1
      ? t("thread.toc.attachmentImage", "Image attachment")
      : t("thread.toc.attachmentFile", "File attachment");
  }
  return t("thread.toc.attachmentCount", `${totalCount} attachments`, {
...
function outlineItemToTocItem(
  ...
  return {
    id: item.id,
    label:
      item.preview || toAttachmentSummaryLabel(item.attachmentSummary, t),
    role: item.role,
  };
}
...
function TocThreadMention({
  ...
  const label = useThreadTitleDisplayText(
    threadTitle ?? t("thread.toc.agentFallbackName", "Agent"),
  );
  return (
    <span
      className={cn(
      ...
      title={t("thread.toc.threadMentionTitle", `Thread: ${label}`, { label })}
    >
      <PromptMentionIcon
  ...
  return (
    <span className="flex min-w-0 items-start gap-1">
      ...
      <span className="line-clamp-2 min-w-0">
        {body || t("thread.toc.attachmentMessage", "Message")}
      </span>
    </span>
  );
}
...
function useConversationTocItems({
  ...
  const outlineTocItems = useMemo(() => {
    ...
    for (const item of outlineItems) {
      const tocItem = outlineItemToTocItem(item, t);
  ...
  const timelineTocItems = useMemo(() => {
    ...
    for (const row of timelineRows) {
      ...
      const item: TocItem = {
        id: row.id,
        label: toTocLabel({ attachments: row.attachments, t, text: row.text }),
        role: row.role,
      };
      if (row.role === "user") {
        userItems.push(item);
    ...
    >
      {tocVisible ? (
        <div className="relative">
          <button
            type="button"
            aria-label={t("thread.toc.ariaLabel", "Thread table of contents")}
            aria-expanded={open}
            ...
            >
              <div className="rounded-lg border border-border bg-popover p-1 shadow-lg">
                <div className="flex items-center gap-1 pb-1">
                  {hasAgentMessages ? (
                    <TocPanelTab
                      label={t("thread.toc.agentMessagesTab", "Agent messages")}
                      active={activeTab === "agent"}
                      onSelect={() => setTab("agent")}
                    />
                  ) : null}
                  <TocPanelTab
                    label={t("thread.toc.userMessagesTab", "Your messages")}
