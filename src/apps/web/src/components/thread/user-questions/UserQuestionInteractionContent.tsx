  ...
  return (
    <div className="mb-2 flex shrink-0 items-center gap-2">
          ...
          return (
            <TabPill
              ...
              label={
                question.shortLabel ??
                t(
                  "thread.userQuestions.questionFallbackLabel",
                  `Question ${index + 1}`,
                  { number: index + 1 },
                )
              }
      ...
      <span className="shrink-0 text-xs text-muted-foreground">
        {t(
          "thread.userQuestions.questionCounter",
...
function QuestionInputBlock({
  ...
  const options = question.options ?? [];
  const answerAriaLabelPrefix = question.shortLabel ?? question.prompt;
  const freeTextLabel = t(
    "thread.userQuestions.answerAriaLabel",
    `${answerAriaLabelPrefix} answer`,
    { prefix: answerAriaLabelPrefix },
  );
  const otherOptionLabel = t("thread.userQuestions.otherOption", "Other…");
  ...
  return (
    <fieldset disabled={disabled} className="min-w-0">
      ...
      <div className="whitespace-pre-wrap text-sm font-semibold text-foreground">
        {question.prompt}
      </div>
      ...
      {state.otherSelected ? (
        <textarea
          ...
          placeholder={t(
            "thread.userQuestions.freeTextPlaceholder",
...
export function UserQuestionAnswerForm({
  ...
  const mutationErrorMessage = resolvePendingInteraction.error
    ? getMutationErrorMessage({
        ...
        fallbackMessage: t(
          "thread.userQuestions.submitFailed",
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex shrink-0 items-center justify-between gap-2">
        ...
        >
          {t("thread.userQuestions.cancel", "Cancel")}
        </Button>
        <div className="flex items-center gap-2">
          {!isFirst ? (
            ...
            >
              {t("thread.userQuestions.back", "Back")}
            </Button>
          ) : null}
          <Button
            type="button"
          ...
          >
            ...
            {isLast
              ? t("thread.userQuestions.submitAnswer", "Submit answer")
              : t("thread.userQuestions.next", "Next")}
          </Button>
        </div>
      </div>
