...
function TimelineUnreadDivider({ autoScroll }: TimelineUnreadDividerProps) {
  ...
  return (
    <div
      ref={dividerRef}
      role="separator"
      aria-label={t("thread.timeline.rows.unreadDivider.ariaLabel", "New messages")}
      className={cn(
    ...
    >
      <span className="shrink-0">
        {t("thread.timeline.rows.unreadDivider.label", "New")}
      </span>
...
function LazyTurnRowBody({
  ...
  if (!rows && isError) {
    return (
      <div className="flex items-center gap-2 text-sm text-destructive-text">
        <span>
          {t(
            "thread.timeline.rows.turnDetails.loadFailed",
        ...
        >
          <Icon name="RotateCcw" />
          {t("thread.timeline.rows.turnDetails.retryButton", "Retry")}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="text-sm text-muted-foreground">
      {t("thread.timeline.rows.turnDetails.loading", "Loading turn details...")}
    </div>
  );
}
