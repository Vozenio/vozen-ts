  ...
  switch (decoration.kind) {
    ...
    case "summary-status": {
      ...
      if (decoration.errorCount > 0) {
        parts.push(
          decoration.errorCount === 1
            ? t(
                "thread.timeline.titleView.errorCountOne",
                `${decoration.errorCount} error`,
                { count: decoration.errorCount },
              )
            : t(
                "thread.timeline.titleView.errorCountMany",
                `${decoration.errorCount} errors`,
                { count: decoration.errorCount },
              ),
        );
      }
      if (decoration.interruptedCount > 0) {
        parts.push(
          t(
            "thread.timeline.titleView.interruptedCount",
            `${decoration.interruptedCount} interrupted`,
            { count: decoration.interruptedCount },
          ),
        );
      }
    ...
    >
      ...
      {title.decorations.map((decoration, index) => (
        <Fragment key={`decoration-${index}`}>
          {" "}
          {renderDecoration(decoration, index, title.tone, t)}
        </Fragment>
      ))}
    </span>
  );
