...
export function ThreadContextWindowIndicator({
  ...
  const titleLabel = usage.estimated
    ? t("thread.timeline.contextWindow.estimatedTitle", "Estimated context")
    : t("thread.timeline.contextWindow.title", "Context window");
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t(
            "thread.timeline.contextWindow.ariaLabel",
            `Context window ${usedPercent}% used`,
            { percent: usedPercent },
          )}
        >
          <svg
        ...
        <div className="space-y-2 max-md:space-y-3">
          <div className="flex items-baseline justify-between gap-2 text-xs max-md:text-sm">
            ...
            <span className={cn("font-medium tabular-nums", toneClass)}>
              {t(
                "thread.timeline.contextWindow.usedPercent",
                `${usedPercent}% used`,
                { percent: usedPercent },
              )}
            </span>
          ...
          <div className="flex items-baseline justify-between gap-2 text-xs tabular-nums text-muted-foreground max-md:text-sm">
            <span>
              {t(
                "thread.timeline.contextWindow.tokensUsage",
                `${usedTokensLabel} / ${windowTokensLabel} tokens`,
                { used: usedTokensLabel, window: windowTokensLabel },
              )}
            </span>
            <span>
              {t(
                "thread.timeline.contextWindow.leftPercent",
                `${leftPercent}% left`,
                { percent: leftPercent },
              )}
            </span>
