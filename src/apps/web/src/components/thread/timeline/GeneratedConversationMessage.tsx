...
interface GeneratedConversationTitleArgs {
  ...
  systemMessageSubject: SystemMessageSubject | null;
  /**
   * Optional so `generatedConversationTitle` stays callable without a live
   * i18next instance (unit tests, storybook-less contexts): falls back to
   * the English `defaultValue`, interpolated the same way i18next would.
   */
  t?: TFunction;
}

function interpolateDefaultValue(
  defaultValue: string,
...
const identityT = ((key: string, defaultValue?: string, options?: unknown) =>
  interpolateDefaultValue(
    defaultValue ?? key,
...
function systemMessageFallbackSegments(t: TFunction): TimelineTitleSegment[] {
  return [
    timelineTitleSegment({
      em: false,
      link: null,
      shimmer: false,
      text: t(
        "thread.timeline.generatedMessage.systemMessageFallbackTitle",
        "System Message",
      ),
      truncate: true,
    }),
...
function threadSubjectTitleSegments(
  ...
  if (subject === null || subject.kind !== "thread") {
    return systemMessageFallbackSegments(t);
  }
  return [
...
function systemMessageTitleSegments(
  ...
  switch (systemMessageKind) {
    case "ownership-assigned":
      return threadSubjectTitleSegments(
        subject,
        t(
          "thread.timeline.generatedMessage.verbs.assignedToYou",
          "assigned to you",
        ),
        t,
      );
    case "ownership-removed":
      return threadSubjectTitleSegments(
        subject,
        t("thread.timeline.generatedMessage.verbs.unassigned", "unassigned"),
        t,
      );
    case "child-needs-attention":
      return threadSubjectTitleSegments(
        subject,
        t(
          "thread.timeline.generatedMessage.verbs.needsAttention",
          "needs attention",
        ),
        t,
      );
    case "child-completed":
      return threadSubjectTitleSegments(
        subject,
        t("thread.timeline.generatedMessage.verbs.finished", "finished"),
        t,
      );
    case "child-failed":
      return threadSubjectTitleSegments(
        subject,
        t("thread.timeline.generatedMessage.verbs.failed", "failed"),
        t,
      );
    case "child-interrupted":
      return threadSubjectTitleSegments(
        subject,
        t(
          "thread.timeline.generatedMessage.verbs.wasInterrupted",
          "was interrupted",
        ),
        t,
      );
    case "child-outcome-batch":
      return subject !== null && subject.kind === "thread-batch"
        ? [
            verbSegment(
              t(
                "thread.timeline.generatedMessage.verbs.threadsUpdated",
                `${subject.count} threads updated`,
                { count: subject.count },
              ),
            ),
          ]
        : systemMessageFallbackSegments(t);
    case "unlabeled":
      return systemMessageFallbackSegments(t);
  }
}
...
export function generatedConversationTitle({
  ...
  const agentLeadIn = sourceIsPluginSideChat
    ? t("thread.timeline.generatedMessage.leadIn.replyingTo", "Replying to")
    : originKind === "fork"
      ? t("thread.timeline.generatedMessage.leadIn.forkedFrom", "Forked from")
      : t("thread.timeline.generatedMessage.leadIn.messageFrom", "Message from");
  ...
  const segments: TimelineTitleSegment[] =
    sourceKind === "agent"
      ...
      : systemMessageTitleSegments(systemMessageKind, systemMessageSubject, t);
...
function generatedConversationEmptyText(
  ...
  switch (sourceKind) {
    case "agent":
      return t(
        "thread.timeline.generatedMessage.emptyState.agentMessage",
        "Sent an agent message",
      );
    case "system":
      return t(
        "thread.timeline.generatedMessage.emptyState.systemMessage",
        "Sent a BB system message",
      );
  }
}
          ...
          <div className="pl-2 text-sm leading-relaxed text-foreground">
            {messageText ? (
              ...
              <p className="text-muted-foreground">
                {generatedConversationEmptyText(sourceKind, t)}
              </p>
            )}
            <ConversationAttachments
              align="start"
