...
export function ThreadTimelinePanelContent({
  ...
  const backgroundOnlyIndicatorLabel =
    displayStatus === "idle" && hasActiveBackgroundWork
      ? t("thread.timeline.panelContent.backgroundWorkRunning", "Background work running")
      : undefined;
  const ongoingIndicatorLabel =
    displayStatus === "host-reconnecting"
      ? t("thread.timeline.panelContent.waitingForReconnection", "Waiting for reconnection")
      : isProvisioningDisplayStatus
        ? t("thread.timeline.panelContent.provisioningThread", "Provisioning thread...")
    ...
    return (
      <ConversationTimeline className="flex-1">
        ...
        <EmptyStatePanel className="mx-2 rounded-lg">
          {t(
            "thread.timeline.panelContent.threadUnavailable",
