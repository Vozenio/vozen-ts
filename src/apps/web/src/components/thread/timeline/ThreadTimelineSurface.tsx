...
function buildStopRequestedTimelineRow({
  ...
  threadId,
  t,
}: BuildStopRequestedTimelineRowArgs & { t: TFunction }): TimelineRow {
  return {
    ...
    title: t("thread.timeline.surface.stopRequestedTitle", "Stop requested"),
    detail: null,
    status: "pending",
    completedAt: null,
  };
...
function useTimelineRowsWithPendingStop({
  ...
  return useMemo(() => {
    ...
    return [
      ...rows,
      buildStopRequestedTimelineRow({ stoppingAnchorAt, threadId, t }),
    ];
  }, [rows, isStopping, stoppingAnchorAt, threadId, t]);
}
    ...
    <ConversationTimeline className="flex-1">
      ...
      {isThreadTimelinePending ? (
        ...
        <TimelineStatusIndicator
          label={t(
            "thread.timeline.surface.failedToLoadTimeline",
    ...
    <div ref={sentinelRef} className="flex justify-center pt-2 mb-3">
      {isAutoLoadEnabled ? (
        ...
        >
          {isLoadingOlderTimelineRows
            ? t(
                "thread.timeline.surface.loadingOlderMessagesStatus",
        ...
        >
          ...
          {isLoadingOlderTimelineRows
            ? t(
                "thread.timeline.surface.loadingOlderMessagesButton",
                "Loading older messages...",
              )
            : t(
                "thread.timeline.surface.loadOlderMessagesButton",
                "Load older messages",
              )}
        </Button>
      )}
...
function ThreadTimelineLoadingSkeleton() {
  ...
  return (
    <div
      ...
      role="status"
      aria-label={t("thread.timeline.surface.loadingThreadAriaLabel", "Loading thread")}
    >
