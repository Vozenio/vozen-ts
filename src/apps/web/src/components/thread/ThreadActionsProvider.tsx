...
export function ThreadActionsProvider({
        ...
        if (shouldShowMutationErrorToast(error)) {
          appToast.error(
            getMutationErrorMessage({
              error,
              fallbackMessage: t(
                "thread.actionsProvider.checkStateFailed",
                "Failed to check thread state",
              ),
            }),
          );
        }
      ...
      archiveThreadAndChildrenMutateAsync({ id: thread.id }).then(
        (response) => {
          ...
          appToast.success(
            ...
            {
              action: {
                label: t("thread.actionsProvider.undo", "Undo"),
                onClick: () => {
        ...
        (error: unknown) => {
          appToast.error(
            getMutationErrorMessage({
              error,
              fallbackMessage: t(
                "thread.actionsProvider.archiveFailed",
                "Failed to archive thread and children",
              ),
  ...
  const toggleRead = useCallback(
          ...
          onError: (error) => {
            appToast.error(
              getMutationErrorMessage({
                error,
                fallbackMessage: t(
                  "thread.actionsProvider.markUnreadFailed",
        ...
        onError: (error) => {
          appToast.error(
            getMutationErrorMessage({
              error,
              fallbackMessage: t(
                "thread.actionsProvider.markReadFailed",
    ...
    [markReadMutate, markUnreadMutate, t],
  );
