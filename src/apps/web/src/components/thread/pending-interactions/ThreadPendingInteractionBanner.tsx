      ...
      return (
        ...
        >
          {sourceThread ? (
            ...
            >
              {t(
                "thread.pendingInteraction.fromChildThread",
...
function PlanReviewRequestBanner({
  ...
  const mutationErrorMessage = resolvePendingInteraction.error
    ? getMutationErrorMessage({
        ...
        fallbackMessage: t(
          "thread.pendingInteraction.planReviewFailed",
          "Failed to resolve plan review",
        ),
  ...
  return (
    <BannerShell
      title={
        approval.reason ??
        t("thread.pendingInteraction.readyToCode", "Ready to code?")
      }
...
function ApprovalPendingInteractionBanner({
  ...
  const submittedDecision = approvalResolutionDecision(interaction.resolution);
  const view = useMemo(
    () => buildApprovalSubject({ interaction, payload, subject, t }),
    [interaction, payload, subject, t],
  );
  const mutationErrorMessage = resolvePendingInteraction.error
    ? getMutationErrorMessage({
        error: resolvePendingInteraction.error,
        fallbackMessage: t(
          "thread.pendingInteraction.resolveFailed",
...
function ApprovalDecisionButton({
  ...
  return (
    ...
    >
      ...
      {labelForApprovalDecision(decision, subjectKind, t)}
    </Button>
  );
}
    ...
    >
      ...
      {ask.headline !== null ? (
        <p className="mt-1 text-xs text-muted-foreground">
          {t("thread.pendingInteraction.toolLabel", `Tool: ${ask.tool}`, {
            tool: ask.tool,
          })}
        </p>
      ) : null}
    ...
    case "command": {
      ...
      return {
        title:
          payload.reason ??
          t(
            "thread.pendingInteraction.confirmCommand",
    ...
    case "file_change": {
      ...
      return {
        title:
          payload.reason ??
          t(
            "thread.pendingInteraction.confirmFileChange",
    ...
    case "permission_grant": {
      ...
      return {
        title:
          payload.reason ??
          t(
            "thread.pendingInteraction.confirmPermissionGrant",
...
function labelForApprovalDecision(
  ...
  if (subjectKind === "plan") {
    return decision === "deny"
      ? t("thread.pendingInteraction.decision.keepPlanning", "Keep planning")
      : t("thread.pendingInteraction.decision.approvePlan", "Approve plan");
  }
  switch (decision) {
    case "allow_once":
      return t("thread.pendingInteraction.decision.allowOnce", "Allow once");
    case "allow_for_session":
      return t(
        "thread.pendingInteraction.decision.allowForSession",
        "Allow for session",
      );
    case "deny":
      return t("thread.pendingInteraction.decision.deny", "Deny");
  }
}
