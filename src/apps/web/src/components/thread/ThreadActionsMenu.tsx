    ...
    <>
      ...
      {onOpenInSplit ? (
        <>
          ...
          >
            {t("thread.actionsMenu.openInSplit", "Open in split")}
          </ThreadActionMenuItem>
          {showSeparators ? (
            <ThreadActionMenuSeparator surface={surface} />
          ) : null}
      ...
      >
        {isRead
          ? t("thread.actionsMenu.markUnread", "Mark unread")
          : t("thread.actionsMenu.markRead", "Mark read")}
      </ThreadActionMenuItem>
      <ThreadActionMenuItem
        surface={surface}
      ...
      >
        {isPinned
          ? t("thread.actionsMenu.unpin", "Unpin")
          : t("thread.actionsMenu.pin", "Pin")}
      </ThreadActionMenuItem>
      <ThreadActionMenuItem
        surface={surface}
        icon="Edit"
      ...
      >
        {t("thread.actionsMenu.rename", "Rename")}
      </ThreadActionMenuItem>
      ...
      >
        {isArchived
          ? t("thread.actionsMenu.unarchive", "Unarchive")
          : t("thread.actionsMenu.archive", "Archive")}
      </ThreadActionMenuItem>
      <ThreadActionMenuItem
        surface={surface}
        icon="Trash2"
      ...
      >
        {t("thread.actionsMenu.delete", "Delete")}
      </ThreadActionMenuItem>
    </>
  );
}
...
export function ThreadArchiveQuickAction({
  ...
  const ariaLabel = isArchived
    ? t("thread.actionsMenu.unarchiveThreadAriaLabel", "Unarchive thread")
    : t("thread.actionsMenu.archiveThreadAriaLabel", "Archive thread");
  return (
    <Tooltip>
    ...
    <DropdownMenu onOpenChange={onOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={t("thread.actionsMenu.ariaLabel", "Thread actions")}
          onClick={(event) => {
            event.stopPropagation();
          }}
        >
...
function ThreadActionsCompactLongPressMenu({
  ...
  return (
    <CompactLongPressMenu
      label={t("thread.actionsMenu.ariaLabel", "Thread actions")}
