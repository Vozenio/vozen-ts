...
function SidebarTriggerOverlay({
  ...
  const triggerProps = {
    "aria-label": shortcut
      ? t("layout.toggleSidebarWithShortcut", { shortcut: shortcut.label })
...
function resolveRouteTitle(
  ...
  const routeTitles: Record<string, { title: string }> = {
    "/": { title: "vozen" },
    "/settings": { title: t("layout.routeTitleSettings") },
    "/automations": { title: t("layout.routeTitleAutomations") },
    "/skills": { title: t("layout.routeTitleSkills") },
  };
  ...
  const actions = pluginPanel ? (
    ...
    <>
      <Link
        ...
        aria-label={t("layout.projectSettings")}
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const threadDisplayTitle = thread
    ...
    : threadId
      ? t("layout.threadIdFallback", { id: threadId.slice(0, 8) })
      : t("layout.threadFallback");
        ...
        : isArchivedView && projectId
          ? isProjectlessProjectId(projectId)
            ? {
                ...
                breadcrumbs: [
                  {
                    label: t("layout.breadcrumbThreads"),
                    to: getRootComposeRoutePath(),
                  },
                  ...(archivedSectionName
                    ? [{ label: archivedSectionName }]
                    : []),
                  { label: t("layout.breadcrumbArchived") },
                ],
              }
            : {
                title: "",
          ...
          : isSettingsView && projectId
            ? {
                ...
                breadcrumbs: [
                  ...
                  { label: t("layout.breadcrumbSettings") },
                ],
              }
            : projectId
              ? {
              ...
              : (resolveRouteTitle(location.pathname, t) ?? { title: "" });
  ...
  const documentTitle = (() => {
    ...
    if (documentTitleBreadcrumbs) {
      const sectionLabel = documentTitleBreadcrumbs[0]?.label ?? "vozen";
    ...
    if (isArchivedView && projectId) {
      if (isProjectlessProjectId(projectId)) {
        return archivedSectionName
          ? t("layout.documentTitleArchivedSuffix", {
              name: archivedSectionName,
            })
          : t("layout.documentTitleThreadsArchived");
      }
      return t("layout.documentTitleArchivedSuffix", {
        name: projectLabel ?? projectId,
      });
    }
    if (isSettingsView && projectId) {
      return t("layout.documentTitleSettingsSuffix", {
        name: projectLabel ?? projectId,
      });
    }
    if (projectId) {
    ...
    const routeTitle = resolveRouteTitle(location.pathname, t)?.title;
    return routeTitle && routeTitle.length > 0 ? routeTitle : "vozen";
  })();
