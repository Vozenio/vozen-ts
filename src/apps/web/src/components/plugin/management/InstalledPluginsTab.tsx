...
export function InstalledPluginsTab({
  ...
  if (plugins.length === 0) {
    return (
      <EmptyState
        message={t(
          "plugin.management.installedPluginsTab.emptyState",
          "No plugins installed. Browse the catalog, create a plugin, or run vozen plugin install <source>.",
        )}
      />
    );
  }
...
export function InstalledPluginRow({
  ...
  const toggle = useMutation({
    ...
    onError: (error, enabled) => {
      appToast.error(
        t(
          "plugin.management.installedPluginsTab.toastToggleFailed",
          "{{action}} {{id}} failed",
          {
            action: enabled
              ? t(
                  "plugin.management.installedPluginsTab.actions.enabling",
                  "Enabling",
                )
              : t(
                  "plugin.management.installedPluginsTab.actions.disabling",
                  "Disabling",
                ),
            id: plugin.id,
          },
  ...
  return (
    <div data-testid={`plugin-row-${plugin.id}`}>
      <ResourceRow
        ...
        openLabel={t(
          "plugin.management.installedPluginsTab.row.openLabel",
          "{{name}} plugin details",
        ...
        persistentActions={
          <>
            {notRunning ? (
              ...
              >
                {t(
                  "plugin.management.installedPluginsTab.row.notRunningBadge",
                  "not running",
                )}
              </span>
            ) : null}
            <Switch
              ...
              aria-label={`${
                enabled
                  ? t(
                      "plugin.management.installedPluginsTab.actions.disable",
                      "Disable",
                    )
                  : t(
                      "plugin.management.installedPluginsTab.actions.enable",
                      "Enable",
                    )
              } ${plugin.id}${
                notRunning
                  ? t(
                      "plugin.management.installedPluginsTab.row.notRunningSuffix",
                      " ({{status}}, not running)",
                      { status: plugin.status },
                    )
                  : ""
