        ...
        <div className="flex items-center justify-end gap-3">
          <div className="flex items-stretch">
            ...
            >
              ...
              {t(
                "plugin.management.browsePluginsTab.createButton.label",
                "Create a plugin",
              )}
            </Button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  aria-label={t(
                    "plugin.management.browsePluginsTab.createButton.optionsAriaLabel",
                    "Create a plugin options",
                  )}
              ...
              <DropdownMenuContent align="end" className="w-max min-w-40">
                <DropdownMenuItem onSelect={onInstallFromSource}>
                  ...
                  {t(
                    "plugin.management.browsePluginsTab.createButton.installFromSource",
                    "Install from source",
                  )}
                </DropdownMenuItem>
              </DropdownMenuContent>
          ...
          <section>
            ...
            <div className="mx-auto w-full max-w-[32rem]">
              <ResourceToolbar
                searchValue={query}
                searchPlaceholder={t(
                  "plugin.management.browsePluginsTab.toolbar.searchPlaceholder",
                  "Search plugins",
                )}
                ...
                controls={
                  <>
                    {categoryOptions.length > 0 ? (
                      <ResourceMultiSelectMenu
                        label={t(
                          "plugin.management.browsePluginsTab.toolbar.categoryLabel",
                          "Category",
                        )}
                      ...
                      options={[
                        {
                          id: "alpha",
                          label: t(
                            "plugin.management.browsePluginsTab.toolbar.sortByNameLabel",
                            "Plugin name",
                          ),
                        },
                      ]}
            ...
            <div className="mt-7 space-y-3">
              {searchQuery.isError && entries.length > 0 ? (
                <p className="text-xs text-warning-text" role="status">
                  {t(
                    "plugin.management.browsePluginsTab.state.cachedResults",
              ...
              {searchQuery.isPending ? (
                <ResourceListState
                  state="loading"
                  message={t(
                    "plugin.management.browsePluginsTab.state.loadingMessage",
                    "Loading plugins",
                  )}
                />
              ) : entries.length === 0 ? (
                <ResourceListState
                  ...
                  message={
                    searchQuery.isError
                      ? t(
                          "plugin.management.browsePluginsTab.state.unavailableMessage",
                          "BB's official plugins are unavailable.",
                        )
                      : t(
                          "plugin.management.browsePluginsTab.state.noSearchMatches",
                ...
                <div className="space-y-3">
                  {groups.length === 0 ? (
                    <ResourceListState
                      state="empty"
                      message={t(
                        "plugin.management.browsePluginsTab.state.noFilterMatches",
                          ...
                          <h2 className="flex items-baseline gap-2 text-sm font-medium text-foreground">
                            ...
                            {group.thirdParty ? (
                              <span className="text-2xs font-normal text-subtle-foreground">
                                {t(
                                  "plugin.management.browsePluginsTab.thirdPartyMarketplaceBadge",
...
function BrowseCard({
  ...
  const uninstall = useMutation({
    ...
    onSuccess: () => {
      ...
      appToast.success(
        t(
          "plugin.management.browsePluginsTab.card.toastUninstalled",
          "{{name}} uninstalled",
    ...
    onError: (error) => {
      appToast.error(
        t(
          "plugin.management.browsePluginsTab.card.toastUninstallFailed",
          "Uninstalling {{name}} failed",
          { name: entry.displayName },
        ),
        {
  ...
  const byline =
    !entry.compatible && entry.incompatibleReason !== null ? (
      ...
      <span>
        {t(
          "plugin.management.browsePluginsTab.card.byline",
          "By: {{name}}",
  ...
  const repositoryLink =
    entry.repositoryUrl === null ? null : (
      <a
        ...
        rel="noreferrer"
        aria-label={t(
          "plugin.management.browsePluginsTab.card.repositoryLinkAriaLabel",
      ...
      >
        {t(
          "plugin.management.browsePluginsTab.card.repositoryLinkLabel",
          "repo",
        )}
  ...
  const headerAction =
    installedPluginId !== null ? (
      <ResourceInstallControl
        accessibleLabel={t(
          "plugin.management.browsePluginsTab.card.uninstallAccessibleLabel",
          "Uninstall {{name}}",
          { name: entry.displayName },
        )}
        icon="Check"
        ...
        presentation="icon"
        tooltip={t(
          "plugin.management.browsePluginsTab.card.installedTooltip",
      ...
      <ResourceInstallControl
        accessibleLabel={t(
          "plugin.management.browsePluginsTab.card.installAccessibleLabel",
          "Install {{name}}",
          { name: entry.displayName },
        )}
        ...
        presentation="icon"
        tooltip={t(
          "plugin.management.browsePluginsTab.card.installTooltip",
          "Install {{name}}",
          { name: entry.displayName },
        )}
        onAction={() =>
  ...
  return (
    <>
      <ResourceBrowseCard
        ...
        openLabel={t(
          "plugin.management.browsePluginsTab.card.openLabel",
          "Open {{name}} details",
          { name: entry.displayName },
        )}
      ...
      >
        <ConfirmDeleteDialogContent
          title={t(
            "plugin.management.browsePluginsTab.confirmDialog.title",
            "Uninstall {{name}}?",
            { name: entry.displayName },
          )}
          description={t(
            "plugin.management.browsePluginsTab.confirmDialog.description",
            "The plugin, its installed files, and its settings, secrets, and schedules are removed from this BB host.",
          )}
          confirmLabel={
            uninstall.isPending
              ? t(
                  "plugin.management.browsePluginsTab.confirmDialog.confirmLabelPending",
                  "Uninstalling…",
                )
              : t(
                  "plugin.management.browsePluginsTab.confirmDialog.confirmLabel",
                  "Uninstall",
                )
          }
          pending={uninstall.isPending}
