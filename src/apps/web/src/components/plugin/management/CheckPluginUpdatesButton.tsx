 ...
 * "up to date".
 */
// NOTE (i18n scope): this function's return strings are intentionally left
// in English. Its test (CheckPluginUpdatesButton.test.tsx, out of scope to
// edit) calls it directly as a pure function with the original two-argument
// signature and asserts on the exact composed English strings; translating
// it would require either changing that call signature or losing the
// singular/plural + list composition the tests lock down. Only the
// component's own rendered label/tooltip/toast strings below are
// translated.
...
export function CheckPluginUpdatesButton({
  ...
  const check = useMutation({
    ...
    onError: (error) => {
      appToast.error(
        t(
          "plugin.management.checkPluginUpdatesButton.toastCheckFailed",
  ...
  const label = check.isPending
    ? t(
        "plugin.management.checkPluginUpdatesButton.checkingLabel",
        "Checking for updates…",
      )
    : t(
        "plugin.management.checkPluginUpdatesButton.checkLabel",
        "Check for updates",
      );
  ...
  const button = (
    ...
    >
      ...
      />
      {inline
        ? t("plugin.management.checkPluginUpdatesButton.inlineLabel", "Check")
        : null}
    </Button>
  );
