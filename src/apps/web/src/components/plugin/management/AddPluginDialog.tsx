...
function catalogInstallDescription(
  ...
  if (source.startsWith("builtin:")) {
    return t(
      "plugin.management.addPluginDialog.description.builtin",
      "Install this plugin, bundled with BB.",
    );
  }
  if (source.startsWith("npm:")) {
    return t(
      "plugin.management.addPluginDialog.description.npm",
      "Install this {{publisherLabel}} plugin from its listed npm package.",
      { publisherLabel },
    );
  }
  return t(
    "plugin.management.addPluginDialog.description.repository",
    "Install this {{publisherLabel}} plugin from its listed source repository.",
    { publisherLabel },
  );
}
...
function resolvedSourceRows(
  ...
  if (source.kind === "npm") {
    return [
      {
        label: t(
          "plugin.management.addPluginDialog.sourceRows.npmPackage",
          "npm package",
        ),
      ...
      ...(source.registry === undefined
        ...
        : [
            {
              label: t(
                "plugin.management.addPluginDialog.sourceRows.registry",
                "registry",
              ),
  ...
  return [
    ...
    {
      label: t(
        "plugin.management.addPluginDialog.sourceRows.repository",
        "repository",
      ),
      value: source.url,
      href: source.url,
    ...
    ...(source.subdir === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.subdirectory",
              "subdirectory",
            ),
            value: source.subdir,
          },
    ...
    ...(source.ref === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.ref",
              "ref",
            ),
            value: source.ref,
          },
    ...
    ...(source.range === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.semverRange",
              "semver range",
            ),
            value:
    ...
    ...(source.resolvedTag === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.resolvesToTag",
              "resolves to tag",
            ),
            value: source.resolvedTag,
          },
        ]),
    ...(source.resolvedCommit === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.resolvesToCommit",
              "resolves to commit",
            ),
    ...
    ...(source.unresolvedReason === undefined
      ...
      : [
          {
            label: t(
              "plugin.management.addPluginDialog.sourceRows.notResolved",
              "not resolved",
            ),
            value: source.unresolvedReason,
          },
        ]),
  ];
}
  ...
  if (pending) {
    return (
      <p className="text-2xs text-subtle-foreground" role="status">
        {t(
          "plugin.management.addPluginDialog.thirdPartySource.resolving",
  ...
  if (error !== null && error !== undefined) {
    return (
      <p className="text-2xs text-warning-text" role="status">
        {t(
          "plugin.management.addPluginDialog.thirdPartySource.resolveError",
  ...
  return (
    <div className="space-y-1.5 rounded-md border border-border bg-muted/30 px-3 py-2">
      <p className="text-2xs text-subtle-foreground">
        {t(
          "plugin.management.addPluginDialog.thirdPartySource.listedByPrefix",
          "Listed by",
        )}{" "}
        <span className="text-foreground">{plan.marketplaceDisplayName}</span>
        {t(
          "plugin.management.addPluginDialog.thirdPartySource.listedBySuffix",
      ...
      <dl className="space-y-0.5">
        <div className="flex gap-2">
          <dt className="w-28 shrink-0 text-2xs text-subtle-foreground">
            {t(
              "plugin.management.addPluginDialog.thirdPartySource.authorLabel",
              "author",
            )}
          </dt>
        ...
        {resolvedSourceRows(t, plan.resolvedSource).map((row) => (
...
function AddPluginDialogContent({
  ...
  const install = useMutation({
    ...
    onSuccess: (plugin) => {
      ...
      appToast.success(
        t(
          "plugin.management.addPluginDialog.toastInstalled",
          "{{name}} installed",
          { name: initial?.displayName ?? "Plugin" },
        ),
      );
      onOpenChange(false);
      onInstalled?.(plugin);
    ...
    onError: (error) => {
      appToast.error(
        t(
          "plugin.management.addPluginDialog.toastInstallFailed",
    ...
    <>
      <DialogHeader>
        <DialogTitle>
          {initial !== null
            ? t(
                "plugin.management.addPluginDialog.installTitle",
                "Install {{name}}?",
                { name: initial.displayName },
              )
            : t("plugin.management.addPluginDialog.addTitle", "Add plugin")}
        </DialogTitle>
        <DialogDescription>
          {initial === null
            ? t(
                "plugin.management.addPluginDialog.descriptionAddPlugin",
                "Install from npm, a Git repository, or a local path.",
              )
            : thirdParty
              ? t(
                  "plugin.management.addPluginDialog.descriptionThirdParty",
      ...
      <div className="space-y-3">
        {initial !== null ? (
          ...
          <div>
            <Input
              ...
              autoFocus
              placeholder={t(
                "plugin.management.addPluginDialog.sourceInput.placeholder",
                "https://github.com/owner/vozen-plugin-name",
              )}
              aria-label={t(
                "plugin.management.addPluginDialog.sourceInput.ariaLabel",
                "Plugin source",
              )}
            ...
            <p className="mt-1.5 text-2xs text-subtle-foreground">
              {t(
                "plugin.management.addPluginDialog.sourceInput.helper",
        ...
        {install.isPending ? (
          <div
            ...
            role="progressbar"
            aria-label={t(
              "plugin.management.addPluginDialog.installingProgressAriaLabel",
              "Installing plugin",
            )}
          >
      ...
      <DialogFooter>
        ...
        >
          {t("plugin.management.addPluginDialog.cancelButton", "Cancel")}
        </Button>
        <Button
          type="button"
          disabled={
        ...
        >
          ...
          {install.isPending
            ? t(
                "plugin.management.addPluginDialog.installingButton",
                "Installing {{name}}…",
                { name: initial?.displayName ?? "plugin" },
              )
            : t(
                "plugin.management.addPluginDialog.installButton",
                "Install {{name}}",
                { name: initial?.displayName ?? "plugin" },
              )}
        </Button>
