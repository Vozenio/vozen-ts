  ...
  const installedActions = (
    <>
      ...
      <CreateWithTemplatesButton
        kind="plugin"
        label={t("plugin.overview.actions.newPlugin")}
        menuActions={[
          {
            label: t("plugin.overview.actions.installFromSource"),
            icon: "Download",
      ...
      <ResourceCollectionViewport
        ...
        toolbar={
          <ResourceToolbar
            ...
            searchPlaceholder={t("plugin.overview.toolbar.searchPlaceholder")}
            onSearchChange={setInstalledQuery}
            action={installedActions}
            controls={
              <>
                <ResourceMultiSelectMenu
                  label={t("plugin.overview.toolbar.typeFilterLabel")}
                ...
                <ResourceSortMenu
                  ...
                  options={[
                    {
                      id: "alpha",
                      label: t("plugin.overview.toolbar.sortByNameLabel"),
                    },
                  ]}
                  onChange={() =>
      ...
      >
        <div className={cn("space-y-3", TOOLS_PAGE_BAND_CLASSES)}>
          {listQuery.isError ? (
            <ResourceListState
              state="error"
              message={t("plugin.overview.list.loadError")}
              onRetry={() => void listQuery.refetch()}
            />
          ) : listQuery.isFetching && listQuery.data === undefined ? (
            <ResourceListState
              state="loading"
              message={t("plugin.overview.list.loading")}
            />
            ...
            <ResourceListState
              ...
              message={
                normalizedInstalledQuery === ""
                  ? t("plugin.overview.list.emptyFiltered")
                  : activeTypeFilters.length > 0
                    ? t("plugin.overview.list.emptySearchWithFilters", {
                        query: installedQuery,
                      })
                    : t("plugin.overview.list.emptySearch", {
                        query: installedQuery,
                      })
              }
            />
