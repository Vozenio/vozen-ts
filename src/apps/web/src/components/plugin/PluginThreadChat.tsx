...
function PluginThreadChatBody({
    ...
    if (environment === null) {
      return (
        <ThreadEnvironmentSummary
          environmentLabel={t("plugin.threadChat.environment.workingLocally")}
          environmentCompactLabel={t("plugin.threadChat.environment.local")}
        />
      );
    }
  ...
  }, [environment, isLocalDaemonHost, t]);
  ...
  if (isThreadMissing) {
    return (
      <EmptyStatePanel className="m-2 rounded-lg">
        {t("plugin.threadChat.threadUnavailable")}
      </EmptyStatePanel>
    );
  }
  if (thread === undefined) {
