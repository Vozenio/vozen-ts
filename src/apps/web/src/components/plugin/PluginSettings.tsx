...
function PluginSettingField({
  ...
  if (descriptor.type === "select") {
    ...
    return (
      <SettingOptionPicker
        ...
        valueLabel={
          value.length > 0 ? value : t("plugin.settings.field.selectPlaceholder")
        }
  ...
  if (descriptor.type === "project") {
    ...
    const valueLabel =
      ...
      (value.length > 0
        ? value
        : t("plugin.settings.field.selectProjectPlaceholder"));
    return (
      <SettingOptionPicker
        ariaLabel={descriptor.label}
  ...
  return (
    <Input
      ...
      placeholder={
        isSecret
          ? secretIsSet
            ? t("plugin.settings.field.secretSet")
            : t("plugin.settings.field.secretNotSet")
          : undefined
      }
      onChange={(event) => onChange(event.target.value)}
...
export function PluginSettingsForm({ pluginId }: { pluginId: string }) {
  ...
  const save = useMutation({
    ...
    onSuccess: (view) => {
      ...
      setDrafts({});
      appToast.success(t("plugin.settings.toast.saveSuccess"));
    },
    onError: (error) => {
      appToast.error(t("plugin.settings.toast.saveError"), {
    ...
    >
        ...
        <SettingsWithControl
          ...
          labelBadge={
            descriptor.type === "string" && descriptor.secret === true
              ? t("plugin.settings.form.secretBadge")
              : undefined
          }
      ...
      <div className="flex justify-end">
        ...
        >
          ...
          {t("plugin.settings.form.saveButton")}
        </Button>
      </div>
    </form>
  );
  ...
  if (listQuery.isFetching && listQuery.data === undefined) {
    return (
      <p className="text-sm text-muted-foreground" role="status">
        {t("plugin.settings.page.loading")}
      </p>
    );
  }
  if (plugin === null) {
    return (
      <p className="text-sm text-muted-foreground" role="status">
        {t("plugin.settings.page.notInstalled")}
      </p>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      ...
      <ResourceDetailStack className="mt-6">
        <ResourceDetailConfigurationSection
          label={t("plugin.settings.page.configurationLabel")}
        >
          <PluginSettingsDetail plugin={plugin} />
        </ResourceDetailConfigurationSection>
        <ResourceDetailOverviewSection
          label={t("plugin.settings.page.pluginDetailsLabel")}
        >
          <p className="max-w-none text-sm leading-relaxed text-muted-foreground">
            {t("plugin.settings.page.detailsLinkPrefix")}{" "}
            <Link
            ...
            >
              {t("plugin.settings.page.detailsLinkText")}
              <Icon
                name="ChevronRight"
        ...
        <ResourceDetailPanel surface="recessed" className="px-3 py-3">
          {settingsAvailable ? (
            ...
            <p className="text-xs text-muted-foreground">
              {plugin.enabled
                ? t("plugin.settings.detail.unavailableWhileStatus", {
                    status: plugin.status,
                  })
                : t("plugin.settings.detail.enableToEdit")}
            </p>
          )}
        </ResourceDetailPanel>
      ) : null}
