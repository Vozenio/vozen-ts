...
export function pluginRemovalLabel(plugin: PluginListItem): string {
  return pluginIsLocalSource(plugin)
    ? i18next.t("tools.pluginDetail.removeFromBb", "Remove from vozen")
    : i18next.t("tools.pluginDetail.uninstall", "Uninstall");
}
...
export function pluginRemovalDescription(plugin: PluginListItem): string {
  return pluginIsLocalSource(plugin)
    ? i18next.t(
        "tools.pluginDetail.removeLocalDescription",
        'Remove "{{id}}" from vozen and delete its settings, secrets, and schedules? Its source files stay on disk. To move it to another directory, install the new path instead; that keeps its settings.',
        { id: plugin.id },
      )
    : i18next.t(
        "tools.pluginDetail.uninstallDescription",
        'Uninstall "{{id}}" and delete its managed files, settings, secrets, and schedules?',
        { id: plugin.id },
      );
}
...
function PluginPath({ path }: { path: string }) {
  ...
  const { copied, copy } = useClipboardCopy({
    text: path,
    errorMessage: t("tools.pluginDetail.copyPathFailed", "Failed to copy path."),
  });
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            type="button"
            aria-label={t(
              "tools.pluginDetail.copyPathAriaLabel",
              "Copy plugin path: {{path}}",
              { path },
            )}
            onClick={() => void copy()}
        ...
        <TooltipContent>
          {copied
            ? t("tools.pluginDetail.copied", "Copied")
            : t("tools.pluginDetail.copyPath", "Copy path")}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
  ...
  return (
    <ResourceDetailPage
      ...
      actions={
        <ResourceInstallControl
          accessibleLabel={t(
            "tools.pluginDetail.installAccessibleLabel",
            "Install {{name}}",
    ...
    >
      <ResourceDetailStack>
        <ResourceDetailOverviewSection
          label={t("tools.pluginDetail.aboutLabel", "About")}
        >
          <p className="max-w-none text-sm leading-relaxed text-muted-foreground">
            {entry.description.length > 0
              ? entry.description
              : t(
                  "tools.pluginDetail.doesNotDescribeItself",
...
export function CatalogPluginDetailBanner({
  ...
  return (
    <PluginBannerBar
      ...
      icon="AlertTriangle"
      title={t(
        "tools.pluginDetail.updateBbToInstall",
        "Update vozen to install this plugin",
      )}
...
export function PluginDetail({
  ...
  if (isLoading) {
    return (
      <ResourceListState
        state="loading"
        message={t("tools.pluginDetail.loadingPlugins", "Loading plugins")}
        layout="detail"
        maxWidthClassName="max-w-5xl"
      />
    );
  ...
  if (plugin === null) {
    return (
      <ResourceListState
        state="empty"
        message={t("tools.pluginDetail.pluginNotFound", "Plugin not found.")}
        layout="detail"
        maxWidthClassName="max-w-5xl"
      />
    );
  ...
  const installedValue = updatesWithBb
    ? t("tools.pluginDetail.updatesWithBb", "Updates with vozen")
    : installedAt !== null
      ? formatAbsoluteDate(installedAt)
      : sourceQuery.isPending
        ? t("tools.pluginDetail.loadingEllipsis", "Loading…")
        : t(
            "tools.pluginDetail.installDateUnavailable",
  ...
  const overflowItems: ResourceOverflowMenuItem[] = [
    ...(canEditSource
      ? [
          {
            label: t("tools.pluginDetail.edit", "Edit"),
            icon: "Edit" as const,
            disabled: pending,
            onSelect: () => onEdit(plugin),
          },
          {
            label: t("tools.pluginDetail.openSource", "Open source"),
            icon: "ExternalLink" as const,
            disabled: pending || openSourceDisabled,
            disabledReason: openSourceDisabled
              ? t("tools.pluginDetail.noEditorConfigured", "No editor configured")
              : undefined,
      ...
      : []),
    {
      ...
      disabledReason:
        plugin.provenance === "builtin"
          ? t(
              "tools.pluginDetail.builtinDisableInstead",
  ...
  return (
    <ResourceDetailPage
      ...
      lifecycleControl={
        <Switch
          ...
          aria-label={
            plugin.enabled
              ? t("tools.pluginDetail.disableAriaLabel", "Disable {{name}}", {
                  name: pluginName,
                })
              : t("tools.pluginDetail.enableAriaLabel", "Enable {{name}}", {
                  name: pluginName,
                })
          }
      ...
      overflowMenu={
        <ResourceOverflowMenu
          label={t("tools.pluginDetail.actionsAriaLabel", "{{name}} actions", {
            name: pluginName,
          })}
          items={overflowItems}
        />
    ...
    >
      <ResourceDetailStack>
        ...
        {hasConfiguration ? (
          ...
          >
            ...
            <p className="max-w-none text-sm leading-relaxed text-muted-foreground">
              ...
              >
                {t(
                  "tools.pluginDetail.itsSettingsPage",
                  "its Settings page",
                )}
                <Icon
                  name="ChevronRight"
        ...
        <ResourceDetailReleaseSection
          label={t("tools.pluginDetail.releaseLabel", "Release")}
          actions={
            hasReleaseControl ? (
        ...
        >
          <PluginDetailTable>
            <PluginDetailFieldRow
              label={
                updatesWithBb
                  ? t("tools.pluginDetail.deliveryLabel", "Delivery")
                  : t("tools.pluginDetail.installedLabel", "Installed")
              }
            ...
            <PluginDetailFieldRow
              label={t("tools.pluginDetail.versionLabel", "Version")}
              labelClassName="font-medium"
            >
            ...
            {hasReleaseUpdate ? (
              <PluginDetailFieldRow
                label={t("tools.pluginDetail.updateLabel", "Update")}
                stackOnNarrow>
        ...
        */}
        {plugin.services.length > 0 ? (
          <ResourceActivitySection
            label={t(
              "tools.pluginDetail.backgroundServicesLabel",
              "Background services",
            )}
          >
        ...
        {plugin.schedules.length > 0 ? (
          <ResourceActivitySection
            label={t("tools.pluginDetail.scheduledJobsLabel", "Scheduled jobs")}
          >
