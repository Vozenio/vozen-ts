...
function SkillPath({ path, href }: { path: string; href?: string }) {
  ...
  const { copied, copy } = useClipboardCopy({
    text: path,
    errorMessage: t("tools.skillDetailView.copyPathFailed", "Failed to copy path."),
  });
  ...
  return (
    <TooltipProvider delayDuration={250}>
      <Tooltip>
        <TooltipTrigger asChild>
          {href ? (
            <a
              ...
              rel="noreferrer"
              aria-label={t(
                "tools.skillDetailView.openInNewTabAriaLabel",
            ...
            <button
              type="button"
              aria-label={t(
                "tools.skillDetailView.copyPathAriaLabel",
                "Copy skill path: {{path}}",
                { path },
              )}
              onClick={() => void copy()}
              className="group -ml-1.5 inline-flex max-w-full cursor-pointer items-center gap-1 rounded-md px-1.5 py-1 text-xs text-subtle-foreground transition-colors hover:bg-state-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
        ...
        <TooltipContent>
          {href
            ? t("tools.skillDetailView.openOnSkillsSh", "Open on skills.sh")
            : copied
              ? t("tools.skillDetailView.copied", "Copied")
              : t("tools.skillDetailView.copyPath", "Copy path")}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
  ...
  return (
    ...
    >
      <ResourceDetailStack>
        {files.length > 1 ? (
          <ResourceDetailIncludesSection
            label={t("tools.skillDetailView.filesLabel", "Files")}
          >
            <SkillFileList
              files={files}
        ...
        <ResourceDefinitionSection label={selectedDisplayPath}>
          {contentState.kind === "loading" ? (
            ...
            >
              {t("tools.skillDetailView.loadingPath", "Loading {{path}}…", {
            ...
            >
              ...
              >
                {t("tools.skillDetailView.retry", "Retry")}
              </Button>
            </ResourceDetailPanel>
          ) : (
