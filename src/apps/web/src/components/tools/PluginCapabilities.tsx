...
function pluginActivityIcon(state: "running" | "ok" | "error" | null): {
  ...
  if (state === "running") {
    ...
    return {
      ...
      label: i18next.t("tools.pluginCapabilities.activity.running", "Running"),
    };
  }
  if (state === "ok") {
    return {
      ...
      label: i18next.t(
        "tools.pluginCapabilities.activity.succeeded",
        "Succeeded",
      ),
    };
  }
  if (state === "error") {
    return {
      ...
      label: i18next.t("tools.pluginCapabilities.activity.failed", "Failed"),
    };
  }
  return {
    name: "Clock",
    className: "text-muted-foreground",
    label: i18next.t("tools.pluginCapabilities.activity.scheduled", "Scheduled"),
  };
}
...
function pluginServiceStatus(state: "running" | "backoff" | "stopped"): {
  ...
  if (state === "running") {
    return {
      label: i18next.t("tools.pluginCapabilities.service.running", "Running"),
  ...
  if (state === "backoff") {
    return {
      label: i18next.t(
        "tools.pluginCapabilities.service.restarting",
        "Restarting",
      ),
      labelClassName: "animate-shine",
      tone: "muted",
  ...
  return {
    label: i18next.t("tools.pluginCapabilities.service.stopped", "Stopped"),
...
function pluginAppSurfaceItems(
  ...
  return [
    ...(plugin.hasSettings || settingsSections.length > 0
      ? [
          namedSurface(
            "settings",
            "settings",
            i18next.t("tools.pluginCapabilities.surface.settingsLabel", "Settings"),
            i18next.t(
              "tools.pluginCapabilities.surface.settingsDescription",
    ...
    ...namedSlotItems(
      ...
      "nav",
      i18next.t(
        "tools.pluginCapabilities.surface.navDescription",
    ...
    ...namedSlotItems(
      ...
      "homepage",
      i18next.t(
        "tools.pluginCapabilities.surface.homepageDescription",
    ...
    ...namedSlotItems(
      ...
      "thread-list",
      i18next.t(
        "tools.pluginCapabilities.surface.threadListDescription",
    ...
    ...namedSlotItems(
      ...
      i18next.t(
        "tools.pluginCapabilities.surface.sourceCodeRendererDescription",
    ...
    ...namedSlotItems(
      ...
      "diff-renderer",
      i18next.t(
        "tools.pluginCapabilities.surface.diffRendererDescription",
    ...
    ...namedSlotItems(
      ...
      "thread-panel",
      i18next.t(
        "tools.pluginCapabilities.surface.threadPanelDescription",
    ...
    ...namedSlotItems(
      ...
      "new-thread-panel",
      i18next.t(
        "tools.pluginCapabilities.surface.newThreadPanelDescription",
    ...
    ...namedSlotItems(
      ...
      "input",
      i18next.t(
        "tools.pluginCapabilities.surface.pendingInteractionDescription",
    ...
    ...namedSlotItems(
      ...
      "sidebar",
      i18next.t(
        "tools.pluginCapabilities.surface.sidebarFooterDescription",
    ...
    ...namedSlotItems(
      ...
      "message-action",
      i18next.t(
        "tools.pluginCapabilities.surface.messageActionDescription",
    ...
    ...namedSlotItems(
      ...
      "thread-header",
      i18next.t(
        "tools.pluginCapabilities.surface.threadHeaderDescription",
      ...
      .flatMap((slot) => [
        ...(slot.actions ?? []).map((action) =>
          namedSurface(
            ...
            undefined,
            i18next.t(
              "tools.pluginCapabilities.surface.composerActionDescription",
        ...
        ...(slot.banners ?? []).map((banner) =>
          namedSurface(
            ...
            undefined,
            i18next.t(
              "tools.pluginCapabilities.surface.composerBannerDescription",
        ...
        ...(slot.plusMenu ?? []).map((item) =>
          namedSurface(
            ...
            item.label,
            i18next.t(
              "tools.pluginCapabilities.surface.composerPlusMenuDescription",
        ...
        ...(slot.richText?.effects ?? []).map((effect) =>
          namedSurface(
            ...
            undefined,
            i18next.t(
              "tools.pluginCapabilities.surface.composerRichTextDescription",
    ...
    ...slots.fileOpeners
      ...
      .map((slot) => ({
        ...namedSurface(
          ...
          slot.title,
          i18next.t(
            "tools.pluginCapabilities.surface.fileOpenerDescription",
        ...
        detail: (
          <span className="flex min-w-0 flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
            <span>
              {i18next.t(
                "tools.pluginCapabilities.surface.fileOpenerDetailPrefix",
                "Opens these files in a plugin-provided viewer:",
              )}
            </span>
    ...
    ...slots.messageDirectives
      ...
      .map((slot) => ({
        ...
        label: `::${slot.id}`,
        detail: i18next.t(
          "tools.pluginCapabilities.surface.messageDirectiveDescription",
          "Renders plugin-provided content inside thread messages.",
        ),
        mono: true,
      })),
  ];
...
export function PluginIncludes({ plugin }: { plugin: PluginListItem }) {
  ...
  const categories: Array<{
    ...
    {
      icon: "AppWindow",
      kind: t("tools.pluginCapabilities.kind.appSurface", "App surface"),
      items: appItems,
    },
    {
      icon: "Terminal",
      kind: t("tools.pluginCapabilities.kind.command", "Command"),
      items: plugin.cliCommand
        ? [
            {
              key: plugin.cliCommand.name,
              label: `vozen ${plugin.cliCommand.name}`,
    ...
    {
      icon: "Explore",
      kind: t("tools.pluginCapabilities.kind.skill", "Skill"),
      items: declared("skill"),
    },
    {
      icon: "Toolbox",
      kind: t("tools.pluginCapabilities.kind.agentTool", "Agent tool"),
      items: declared("agent-tool"),
    },
    {
      icon: "MessageCirclePlus",
      kind: t(
        "tools.pluginCapabilities.kind.threadIntegration",
        "Thread integration",
      ),
    ...
    {
      icon: "Palette",
      kind: t("tools.pluginCapabilities.kind.theme", "Theme"),
  ...
  const liveCapabilitiesNote = t(
    "tools.pluginCapabilities.notRunningNote",
    "This plugin isn't running, so its commands, settings, agent tools, app surfaces, and thread integrations can't be listed.",
  );

  return (
    <ResourceDetailIncludesSection
      label={t("tools.pluginCapabilities.sectionLabel", "Capabilities")}
    >
      ...
      action={
        canReload ? (
          ...
          >
            ...
            {reloadPending
              ? t("tools.pluginCapabilities.reloading", "Reloading\u2026")
              : t("tools.pluginCapabilities.reload", "Reload")}
          </Button>
        ) : undefined
      }
    />
...
export function PluginHealthBanner({
  ...
  const reload = useMutation({
    ...
    onError: (error) => {
      appToast.error(
        i18next.t(
          "tools.pluginCapabilities.reloadFailedToast",
...
export function PluginServices({ plugin }: { plugin: PluginListItem }) {
  ...
  return (
    <div className="max-w-full overflow-hidden rounded-lg border border-border bg-card align-top">
      <table
        aria-label={t(
          "tools.pluginCapabilities.backgroundServicesAriaLabel",
          "Background services",
        )}
      ...
      >
        ...
        <thead className="text-xs text-muted-foreground">
          <tr className="border-b border-border">
            ...
            >
              {t("tools.pluginCapabilities.statusColumn", "Status")}
            </th>
            <th
              scope="col"
              className={cn(
            ...
            >
              {t("tools.pluginCapabilities.serviceColumn", "Service")}
            </th>
          </tr>
        </thead>
    ...
    <PluginDetailTable>
      {plugin.schedules.map((schedule) => (
        <PluginDetailRow
          ...
          detail={
            schedule.lastError ??
            t("tools.pluginCapabilities.nextRun", "Next {{date}}", {
              date: formatAbsoluteDate(schedule.nextRunAt),
            })
          }
        />
      ))}
