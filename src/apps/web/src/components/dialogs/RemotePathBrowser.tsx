...
export function getFolderNameValidationMessage(name: string): string | null {
  if (!name || name === "." || name === "..") {
    return (
      i18next.t("dialogs.remotePathBrowser.enterFolderName") ??
      "Enter a folder name."
    );
  }
  if (/[\\/]/.test(name)) {
    return (
      i18next.t("dialogs.remotePathBrowser.noSlashesInName") ??
    ...
    onError: (mutationError) => {
      setNewFolderError(
        getMutationErrorMessage({
          error: mutationError,
          fallbackMessage: t("dialogs.remotePathBrowser.createFolderError"),
        }),
      );
    },
  });
  ...
  if (isError) {
    body = (
      <EmptyState
        ...
        message={getMutationErrorMessage({
          error,
          fallbackMessage: t("dialogs.remotePathBrowser.readFolderError"),
        })}
    ...
    body = (
      <EmptyState
        ...
        message={t("dialogs.remotePathBrowser.loading")}
        className="px-2 py-3"
      />
    );
    ...
    body = (
      <EmptyState
        message={t("dialogs.remotePathBrowser.emptyFolder")}
        className="px-2 py-3"
      />
    );
  } else {
    ...
    <div className="flex flex-col rounded-md border">
      <div className="flex items-center gap-1 border-b px-1.5 py-1">
        {isEditing ? (
          <>
            <Input
              ref={editInputRef}
              aria-label={t("dialogs.remotePathBrowser.pathFieldLabel")}
              className="h-7 flex-1 text-xs"
              value={editValue}
              disabled={disabled}
              placeholder={t("dialogs.remotePathBrowser.pathPlaceholder")}
            ...
            <Button
              ...
              aria-label={t("dialogs.remotePathBrowser.goToPath")}
              disabled={disabled}
          ...
          <>
            <Button
              ...
              size="icon"
              className="size-7 shrink-0"
              aria-label={t("dialogs.remotePathBrowser.goToParent")}
            ...
            {allowCreateFolder ? (
              <Button
                ...
                size="icon"
                className="size-7 shrink-0"
                aria-label={t("dialogs.remotePathBrowser.newFolder")}
            ...
            <Button
              ...
              size="icon"
              className="size-7 shrink-0"
              aria-label={t("dialogs.remotePathBrowser.editPath")}
              disabled={interactionDisabled}
              onClick={openEditor}
            >
              <Icon name="Edit" />
      ...
      >
        {isCreatingFolder ? (
          <div
            role="group"
            aria-label={t("dialogs.remotePathBrowser.createNewFolderGroup")}
          ...
          >
            ...
            <div className="flex items-center gap-2 px-2">
              ...
              <Input
                ...
                aria-label={t("dialogs.remotePathBrowser.newFolderNameLabel")}
                className="-ml-1 h-7 min-w-0 flex-1 px-1 text-sm"
                value={newFolderName}
                disabled={interactionDisabled}
                placeholder={t(
                  "dialogs.remotePathBrowser.folderNamePlaceholder",
                )}
                onChange={(event) => {
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                <Button
                  ...
                  size="icon"
                  className="size-7 shrink-0"
                  aria-label={t("dialogs.remotePathBrowser.createFolder")}
                  disabled={interactionDisabled}
                ...
                <Button
                  ...
                  size="icon"
                  className="size-7 shrink-0"
                  aria-label={t("dialogs.remotePathBrowser.cancelNewFolder")}
                  disabled={interactionDisabled}
