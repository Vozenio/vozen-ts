...
function getDialogTitle(
  t: TFunction,
  kind: ProjectPathDialogTarget["kind"],
): string {
  switch (kind) {
    case "create":
      return t("dialogs.projectPath.title.create");
    case "update":
      return t("dialogs.projectPath.title.update");
    case "add-source":
      return t("dialogs.projectPath.title.addSource");
  }
}
...
function getDialogSubmitLabel(
  ...
  switch (kind) {
    case "create":
      return t("dialogs.projectPath.submitLabel.create");
    case "update":
      return t("dialogs.projectPath.submitLabel.update");
    case "add-source":
      return t("dialogs.projectPath.submitLabel.addSource");
  }
}
  ...
  if (platform === "wsl") {
    return {
      description: hostName
        ? t("dialogs.projectPath.wslDescriptionWithHost", { hostName })
        : t("dialogs.projectPath.wslDescription"),
      placeholder,
    };
  }
  return {
    description: hostName
      ? t("dialogs.projectPath.descriptionWithHost", { hostName })
      : t("dialogs.projectPath.description"),
    placeholder,
  };
}
...
export function ProjectPathDialogContent({
  ...
  const copy = getPlatformCopy(t, platform, selectedHostName);
  const placeholder =
    target.kind === "update"
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedPath) {
      setValidationMessage(t("dialogs.projectPath.chooseFolderError"));
      return;
    }
    ...
    if (
      ...
      setValidationMessage(
        t("dialogs.projectPath.couldNotDeriveNameError"),
      );
      return;
    }
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{getDialogTitle(t, target.kind)}</DialogTitle>
        <DialogDescription>
          {selectedHostId
            ? selectedHostName
              ? t("dialogs.projectPath.browseDescriptionWithHost", {
                  hostName: selectedHostName,
                })
              : t("dialogs.projectPath.browseDescription")
            : noMachineAvailable
              ? t("dialogs.projectPath.noMachineOnline")
              : copy.description}
        </DialogDescription>
      </DialogHeader>
      <form className="min-w-0 space-y-4" onSubmit={handleSubmit}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild disabled={pending}>
              <Button
                type="button"
                variant="outline"
                aria-label={t("dialogs.projectPath.machineLabel")}
                disabled={pending}
              ...
              >
                <span className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="min-w-0 truncate">
                    {selectedHost?.name ??
                      t("dialogs.projectPath.selectMachine")}
                  </span>
                </span>
                <Icon
                  name="ChevronDown"
            ...
            <DropdownMenuContent
              align="start"
              mobileTitle={t("dialogs.projectPath.machineLabel")}
        ...
        {selectedHostId ? (
          ...
          <p className="rounded-md border px-3 py-6 text-center text-sm text-muted-foreground">
            {t("dialogs.projectPath.allMachinesOffline")}
          </p>
        ) : (
          <Input
            id={inputId}
            aria-label={t("dialogs.projectPath.pathFieldLabel")}
            value={manualPath}
            ...
            {target.kind === "create" && derivedProjectName ? (
              <p className="flex min-w-0 gap-1 text-sm text-muted-foreground">
                <span className="shrink-0">
                  {t("dialogs.projectPath.projectNameLabel")}
                </span>
        ...
        <DialogFooter>
          ...
          >
            {getDialogSubmitLabel(t, target.kind)}
          </Button>
        </DialogFooter>
      </form>
    </>
