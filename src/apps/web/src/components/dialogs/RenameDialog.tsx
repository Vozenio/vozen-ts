...
export function RenameDialogContent({
  ...
  const { validationMessage, validate, clearMessage } = useNameValidation({
    emptyMessage: t("dialogs.rename.emptyNameError", {
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>
          {t("dialogs.rename.title", { entity: entityLabel })}
        </DialogTitle>
        <DialogDescription>
          {t("dialogs.rename.description", { entity: entityLabel })}
        </DialogDescription>
      </DialogHeader>
      <form
      ...
      >
        <div className={compact ? "space-y-1.5" : "space-y-2"}>
          <Input
            ref={inputRef}
            id={inputId}
            aria-label={t("dialogs.rename.nameFieldLabel", {
              entity: capitalizedEntityLabel,
            })}
            value={nextName}
