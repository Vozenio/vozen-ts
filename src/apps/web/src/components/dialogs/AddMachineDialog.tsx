  ...
  return (
    ...
    >
      <p className="text-sm text-foreground">
        {t("dialogs.addMachine.unreachableNotice.addressWarning")}
      </p>
      <p className="text-xs text-subtle-foreground">
        {t("dialogs.addMachine.unreachableNotice.targetPrefix")}{" "}
        <span className="font-mono">{serverUrl}</span>
        {t("dialogs.addMachine.unreachableNotice.targetSuffix")}{" "}
        {reason === "disabled"
          ? t("dialogs.addMachine.unreachableNotice.disabledExplanation")
          : t("dialogs.addMachine.unreachableNotice.unpairedExplanation")}
      </p>
      <div className="flex items-center gap-2">
        ...
        >
          {reason === "disabled" ? (
            <Link to={CONNECT_PLUGIN_ROUTE}>
              {t("dialogs.addMachine.unreachableNotice.enableConnectPlugin")}
            </Link>
          ) : (
            <Link to={REMOTE_ACCESS_ROUTE}>
              {t("dialogs.addMachine.unreachableNotice.setupRemoteAccess")}
            </Link>
          )}
        </Button>
        <a
        ...
        >
          {t("dialogs.addMachine.unreachableNotice.otherOptions")}
        </a>
      </div>
    </div>
  );
...
function AddMachineDialogContent({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{t("dialogs.addMachine.title")}</DialogTitle>
        <DialogDescription>
          {unreachable !== null
            ? t("dialogs.addMachine.descriptionPairOnly")
            : t("dialogs.addMachine.descriptionRunCommand")}
         </DialogDescription>
       </DialogHeader>
        ...
        {mintJoinCode.isError || connectUnavailable ? (
          <div className="space-y-2">
            <p className="text-sm text-destructive">
              {connectUnavailable
                ? t("dialogs.addMachine.connectNotReady")
                : getMutationErrorMessage({
                    error: mintJoinCode.error,
                    fallbackMessage: t(
                      "dialogs.addMachine.createJoinCodeError",
                    ),
                  })}
            </p>
            <Button
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-border px-3 py-2">
              {expired ? (
                <>
                  <span className="text-xs text-subtle-foreground">
                    {t("dialogs.addMachine.codeExpired")}
                  </span>
                  <Button
                    type="button"
                    size="sm"
                    ...
                    onClick={() => mintJoinCode.mutate()}
                  >
                    {t("dialogs.addMachine.generateNewCode")}
                  </Button>
                </>
              ) : remainingMs !== null ? (
                <span className="text-xs tabular-nums text-subtle-foreground">
                  {t("dialogs.addMachine.codeExpiresIn", {
              ...
              >
                {copied
                  ? t("dialogs.addMachine.copied")
                  : t("dialogs.addMachine.copy")}
              </Button>
            </div>
          </div>
        ) : (
          <p className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            {t("dialogs.addMachine.creatingJoinCode")}
          </p>
        )}
            ...
            {connectedNewHost !== null ? (
              <>
                ...
                <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                  {t("dialogs.addMachine.hostConnected", {
                    name: connectedNewHost.name,
                  })}
                </span>
                <Button
                  type="button"
                ...
                >
                  {t("dialogs.addMachine.setupProjectOnIt")}
                </Button>
              </>
            ) : (
              <>
                ...
                <span className="text-sm text-muted-foreground">
                  {t("dialogs.addMachine.waitingForMachine")}
                </span>
              </>
            )}
          </div>
