...
const KIND_FILTER_VALUES: readonly ArchivedThreadsKindFilter[] = [
  "all",
  "root",
  "child",
];
  ...
  const kindOptions = useMemo(
    () =>
      KIND_FILTER_VALUES.map((value) => ({
        label:
          value === "all"
            ? t("settingsMisc.archivedThreads.kindAll", "All threads")
            : value === "root"
              ? t("settingsMisc.archivedThreads.kindRoot", "Root threads")
              : t("settingsMisc.archivedThreads.kindChild", "Child threads"),
        value,
      })),
    [t],
  );
  const allProjectsLabel = t(
    "settingsMisc.archivedThreads.allProjects",
    "All projects",
  );
  const projectOptions = useMemo(
    () => [
      { label: allProjectsLabel, value: ALL_PROJECTS },
    ...
    <section className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-foreground">
          {t("settingsMisc.archivedThreads.title", "Archived threads")}
        </h2>
        <p className="mt-1 text-xs text-subtle-foreground">
          {t(
            "settingsMisc.archivedThreads.description",
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-56 flex-1">
          ...
          <Input
            aria-label={t(
              "settingsMisc.archivedThreads.searchAriaLabel",
              "Search archived threads",
            )}
            className="h-8 pl-8 text-xs"
            onChange={(event) => setSearch(event.target.value)}
            placeholder={t(
              "settingsMisc.archivedThreads.searchPlaceholder",
              "Search archived threads…",
            )}
            value={search}
          />
      ...
      {isInitialLoading ? (
        <p className="py-8 text-center text-sm text-muted-foreground">
          {t(
            "settingsMisc.archivedThreads.loading",
        ...
        <EmptyStatePanel className="py-8">
          {search.trim().length > 0
            ? t(
                "settingsMisc.archivedThreads.noMatchSearch",
                "No archived threads match your search.",
              )
            : t(
                "settingsMisc.archivedThreads.noMatchFilters",
            ...
            <section key={groupProjectId} className="space-y-2">
              <div className="flex items-center gap-2 px-0.5 text-xs font-medium text-muted-foreground">
                ...
                <h3 className="min-w-0 flex-1 truncate">
                  {projectNames.get(groupProjectId) ??
                    t(
                      "settingsMisc.archivedThreads.unknownProject",
                      "Unknown project",
                    )}
                </h3>
                <span>
                  {threads.length === 1
                    ? t("settingsMisc.archivedThreads.oneThread", "{{count}} thread", {
                        count: threads.length,
                      })
                    : t(
                        "settingsMisc.archivedThreads.manyThreads",
                        "{{count}} threads",
                        { count: threads.length },
                      )}
                </span>
                      ...
                      <span className="flex min-w-0 items-center gap-2 text-sm">
                        ...
                        {thread.parentThreadId !== null ? (
                          <Pill variant="outline" className="shrink-0">
                            {t("settingsMisc.archivedThreads.childPill", "child")}
                          </Pill>
                        ) : null}
                      </span>
                      {thread.archivedAt !== null ? (
                        <span className="mt-0.5 block text-xs text-subtle-foreground">
                          {t(
                            "settingsMisc.archivedThreads.archivedAt",
      ...
      {!searchIsActive && archivedThreadsQuery.hasNextPage ? (
        ...
        >
          {archivedThreadsQuery.isFetchingNextPage
            ? t("settingsMisc.archivedThreads.loadingMore", "Loading…")
            : t("settingsMisc.archivedThreads.loadMore", "Load more")}
        </Button>
      ) : null}
    </section>
  );
