...
export const SETTINGS_NAV_SECTIONS = [
  ...
  { icon: "Smartphone", id: "remote-access", label: "Remote access" },
...
type SettingsNavSectionStatic = (typeof SETTINGS_NAV_SECTIONS)[number];

export type SettingsSectionId = SettingsNavSectionStatic["id"];

// Widens `label` to plain `string`: useSettingsNavState() replaces the
// static English label with a translated one, which isn't one of the
// literal strings `SettingsNavSectionStatic` pins it to.
type SettingsNavSection = Omit<SettingsNavSectionStatic, "label"> & {
  label: string;
};
...
export function useSettingsNavState(): SettingsNavState {
  ...
  const sections: SettingsNavSection[] = SETTINGS_NAV_SECTIONS.filter(
    (section) => {
      if (section.id === "files") {
        return (
      ...
      // Ported straight from bb's real multi-machine daemon fleet and
      // community links — vozen has neither (see AddMachineDialog.tsx: its
      // RPC always 404s here) and no real repo/Discord of its own to link
      // out to. Left routable rather than deleted (see route-paths.ts) so a
      // stale bookmark to /settings/machines still resolves to something.
      if (section.id === "machines" || section.id === "community") {
        return false;
      }
      return true;
    },
    ...
    ...section,
    label: t(`settingsMisc.nav.${section.id}`, section.label),
  }));
