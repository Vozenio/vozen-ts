...
function providerConfig(
  ...
  return {
    ...
    provider: info,
    signInHint:
      info?.strings?.signInHint ??
      i18next.t(
        "settingsMisc.usageLimits.signInHint",
        "Sign in to {{name}}, then reload usage.",
        { name },
      ),
    expiredHint:
      info?.strings?.expiredHint ??
      i18next.t(
        "settingsMisc.usageLimits.expiredHint",
...
function formatReset(resetsAt: string | null): string | null {
  ...
  if (diffMs <= 0) {
    return i18next.t("settingsMisc.usageLimits.resettingNow", "Resetting now");
  }

  const diffMinutes = Math.round(diffMs / 60_000);
  if (diffMinutes < 60) {
    return i18next.t(
      "settingsMisc.usageLimits.resetsInMinutes",
  ...
  if (diffHours < 24) {
    ...
    return minutes > 0
      ? i18next.t(
          "settingsMisc.usageLimits.resetsInHoursMinutes",
          "Resets in {{hours}} hr {{minutes}} min",
          { hours: diffHours, minutes },
        )
      : i18next.t(
          "settingsMisc.usageLimits.resetsInHours",
          "Resets in {{hours}} hr",
          { hours: diffHours },
        );
  }
  ...
  return i18next.t("settingsMisc.usageLimits.resetsAt", "Resets {{formatted}}", {
    formatted,
  });
}
...
function usageWindowValue(window: ProviderUsageWindow): string {
  if (!window.cost) {
    return i18next.t(
      "settingsMisc.usageLimits.percentUsed",
      "{{percent}}% used",
...
function UsageMachinePicker({
  ...
  const pickerLabel = t(
    "settingsMisc.usageLimits.machinePickerAriaLabel",
    "Usage limits machine",
  );
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span className="min-w-0 truncate">
            {selectedHost?.name ??
              t("settingsMisc.usageLimits.machineFallback", "Machine")}
          </span>
          <Icon name="ChevronDown" className="size-3.5 shrink-0" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" mobileTitle={pickerLabel}>
        {hosts.map((host) => {
...
function ProviderUsageBody({
  ...
  if (isError) {
    return (
      <p className="text-xs text-muted-foreground">
        {t(
          "settingsMisc.usageLimits.loadErrorDescription",
  ...
  if (!usage) {
    return (
      <p className="text-xs text-muted-foreground">
        {isLoading
          ? t("settingsMisc.usageLimits.loadingUsage", "Loading usage…")
          : t("settingsMisc.usageLimits.usageNotProvided", "Usage not provided.")}
      </p>
    );
  }
  switch (usage.status) {
    case "ok":
      if (usage.windows.length === 0) {
        return (
          <p className="text-xs text-muted-foreground">
            {t(
              "settingsMisc.usageLimits.noLimitsReported",
    ...
    case "not_installed":
      return (
        <p className="text-xs text-muted-foreground">
          {t(
            "settingsMisc.usageLimits.notInstalled",
...
export function UsageLimitsSettingsSectionContent({
  ...
  const emptyMessage =
    isLoading || isProviderListLoading
      ? t(
          "settingsMisc.usageLimits.loadingProvidersAndUsage",
          "Loading providers and usage…",
        )
      : isError || isProviderListError
        ? t(
            "settingsMisc.usageLimits.loadProvidersError",
            "Couldn't load providers or usage right now.",
          )
        : t("settingsMisc.usageLimits.noProviders", "No providers available.");
  const reloadLabel = t(
    "settingsMisc.usageLimits.reloadUsageData",
    "Reload usage data",
  );
  return (
    <SettingsSection
      title={t("settingsMisc.usageLimits.title", "Usage limits")}
      description={t(
        "settingsMisc.usageLimits.description",
      ...
      action={
        <div className="flex items-center gap-1">
          ...
          <Tooltip delayDuration={300} disableHoverableContent>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={
                  isFetching
                    ? t(
                        "settingsMisc.usageLimits.reloadingUsageData",
            ...
            <TooltipContent side="bottom">{reloadLabel}</TooltipContent>
          </Tooltip>
        </div>
      }
    >
