  ...
  return (
    ...
    >
      <Button
        ...
        disabled={disabled}
        aria-label={t(
          "settingsMisc.providers.reorderAriaLabel",
          "Reorder {{name}}",
      ...
      {!provider.available ? (
        <SettingsBadge>
          {t("settingsMisc.providers.unavailableBadge", "Unavailable")}
        </SettingsBadge>
      ) : null}
      {isDefault ? (
        <SettingsBadge>
          {t("settingsMisc.providers.defaultBadge", "Default")}
        </SettingsBadge>
      ) : (
        <Button
          variant="ghost"
        ...
        >
          {t("settingsMisc.providers.makeDefault", "Make default")}
        </Button>
      )}
    </SettingsRow>
  );
...
export function ProvidersSettingsSection({
  ...
  return (
    <SettingsSection
      title={t("settingsMisc.providers.title", "Providers")}
      description={t(
        "settingsMisc.providers.description",
        "Set the default agent and its order in provider pickers. Configure each provider on its plugin page under Plugins.",
      )}
    >
      {providersQuery.isPending ? (
        <p className="text-sm text-muted-foreground">
          {t("settingsMisc.providers.loading", "Loading providers…")}
        </p>
      ) : providers.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          {t(
            "settingsMisc.providers.noneEnabled",
