...
function formatWorkspaceChangedFilesLabel(changedFiles: number): string {
  return changedFiles === 1
    ? i18next.t("workspace.changeSummary.oneFile", "{{count}} file", {
        count: changedFiles,
      })
    : i18next.t("workspace.changeSummary.manyFiles", "{{count}} files", {
        count: changedFiles,
      });
}
...
export function formatChangeSummary(tally: ChangeTally): string {
  if (
    ...
    return i18next.t("workspace.changeSummary.noChanges", "No changes");
  }
...
export function selectWorkspaceChangedFilesSections(
  ...
  if (workingTree.files.length > 0) {
    ...
    sections.push({
      ...
      label: isUntrackedOnly
        ? i18next.t("workspace.changeSummary.untracked", "Untracked")
        : i18next.t("workspace.changeSummary.uncommitted", "Uncommitted"),
  ...
  if (mergeBase && mergeBase.files.length > 0) {
    sections.push({
      kind: "committed",
      label: i18next.t("workspace.changeSummary.committed", "Committed"),
