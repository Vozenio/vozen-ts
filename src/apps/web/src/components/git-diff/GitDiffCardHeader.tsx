...
export function GitDiffCardRawToggle({
  ...
  const label = isRaw
    ? t("gitDiff.cardHeader.rawToggle.showPreview", "Show image preview for {{fileLabel}}", { fileLabel })
    : t("gitDiff.cardHeader.rawToggle.showRaw", "Show raw SVG diff for {{fileLabel}}", { fileLabel });
  return (
    <button
      type="button"
        ...
        {supportsCollapse ? (
          <button
            ...
            aria-label={
              hasChanges
                ? isCollapsed
                  ? t("gitDiff.cardHeader.toggle.expand", "Expand {{label}}", {
                      label: model.label,
                    })
                  : t(
                      "gitDiff.cardHeader.toggle.collapse",
                      "Collapse {{label}}",
                      { label: model.label },
                    )
                : t(
                    "gitDiff.cardHeader.toggle.noChanges",
                    "{{label}} has no changes to expand",
                    { label: model.label },
                  )
            }
        ...
        >
          ...
          {canOpenFile && openablePath && onOpenFileInEditor ? (
            <OpenInEditorButton
              ...
              label={t(
                "gitDiff.cardHeader.openInEditor",
                "Open {{label}} in editor",
                { label: model.label },
              )}
            />
          ) : null}
