...
interface GitDiffCardImageSide {
  url: string;
  /** Which half of a two-image diff this is; `null` when there is only one image to show. */
  side: "old" | "new" | null;
}
...
function buildGitDiffCardImageSides(
  ...
  if (oldImageUrl !== null) {
    sides.push({ url: oldImageUrl, side: showSideLabels ? "old" : null });
  }
  if (newImageUrl !== null) {
    sides.push({ url: newImageUrl, side: showSideLabels ? "new" : null });
  }
  return sides;
}

function getGitDiffCardImageSideCaption(
  t: TFunction,
  side: GitDiffCardImageSide,
): string | null {
  if (side.side === null) return null;
  return side.side === "old"
    ? t("gitDiff.cardBody.imagePreview.old", "Old")
    : t("gitDiff.cardBody.imagePreview.new", "New");
}
...
function getGitDiffCardImageAlt(
  ...
  if (side.side === null) return fileDiffLabel;
  return side.side === "old"
    ? t("gitDiff.cardBody.imagePreview.altOld", "{{fileDiffLabel}} (old)", {
        fileDiffLabel,
      })
    : t("gitDiff.cardBody.imagePreview.altNew", "{{fileDiffLabel}} (new)", {
        fileDiffLabel,
      });
}
...
export function GitDiffCardImagePreviewBody({
  ...
  if (imageSides.length === 0) {
    return (
      <div className="px-3 py-3 text-xs text-muted-foreground">
        {t(
          "gitDiff.cardBody.imagePreview.noPreview",
  ...
  return (
    <>
        ...
        {imageSides.map((side, index) => (
          <figure key={side.url} className="min-w-0">
            ...
            >
              <img
                src={side.url}
                alt={getGitDiffCardImageAlt(t, fileDiffLabel, side)}
            ...
            {side.side !== null ? (
              <figcaption className="mt-1 text-xs text-muted-foreground">
                {getGitDiffCardImageSideCaption(t, side)}
              </figcaption>
            ) : null}
          </figure>
        ))}
      ...
      <ImageLightbox
        title={t(
          "gitDiff.cardBody.imagePreview.lightboxTitle",
          "{{fileDiffLabel}} image preview",
          { fileDiffLabel },
        )}
        imageSrc={expandedImageSide?.url ?? null}
        imageAlt={
          expandedImageSide
            ? getGitDiffCardImageAlt(t, fileDiffLabel, expandedImageSide)
            : fileDiffLabel
        }
      ...
      {shouldGateDeletedDiff ? (
        <div className="flex items-center py-3 pl-2 pr-3 text-xs text-muted-foreground">
          ...
          <span className="pl-[1ch]">
            <span>
              {t(
                "gitDiff.cardBody.deletedFile.message",
            ...
            >
              {t("gitDiff.cardBody.deletedFile.loadButton", "Load diff")}
            </Button>
          </span>
        </div>
      ) : !shouldRenderDiffView ? (
    ...
    <div className="flex items-center py-2 pl-2 pr-3 text-xs text-muted-foreground">
      ...
      <span className="pl-[1ch]">
        {status === "loading" ? (
          <span role="status">
            {t(
              "gitDiff.cardBody.contextExpansion.loading",
              "Loading context…",
            )}
          </span>
        ) : (
          <>
            {status === "error" ? (
              <>
                <span className="text-destructive">
                  {t(
                    "gitDiff.cardBody.contextExpansion.error",
            ...
            >
              {status === "error"
                ? t("gitDiff.cardBody.contextExpansion.retryButton", "Retry")
                : t(
                    "gitDiff.cardBody.contextExpansion.expandButton",
                    "Expand context",
                  )}
            </Button>
          </>
