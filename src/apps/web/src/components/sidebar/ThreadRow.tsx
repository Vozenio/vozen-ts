...
import { isHerdrThread } from "@/lib/herdr-thread";
...
function HerdrThreadBadge() {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          role="img"
          aria-label="Herdr agent"
          className="relative top-px flex shrink-0 items-center text-muted-foreground"
        >
          <Icon name="Terminal" className="size-3.5" aria-hidden />
        </span>
      </TooltipTrigger>
      <TooltipContent side="top">Herdr agent</TooltipContent>
    </Tooltip>
  );
}
...
function ThreadRowComponent({
  ...
  const crossProjectLabel =
    crossProjectId === null
      ? null
      : crossProjectName
        ? t("sidebar.thread.inProject", { project: crossProjectName })
        : t("sidebar.thread.inAnotherProject");
  ...
  const splitIndicatorLabel = trailingIndicatorResolution.accessibleLabel
    ? t("sidebar.thread.openInSplitWithStatus", {
        title: labelTitle,
        status: trailingIndicatorResolution.accessibleLabel,
      })
    : t("sidebar.thread.openInSplit", { title: labelTitle });
  const linkLabel = hasComposerDraft
    ? t("sidebar.thread.openWithDraft", { title: labelTitle })
    : t("sidebar.thread.open", { title: labelTitle });
  ...
  const rowContent = (
    <>
      ...
      >
        ...
        {/* Trailing, not leading: a leading badge indents only Herdr rows,
            leaving mixed lists ragged (same slot the cross-project glyph
            uses). */}
        {isHerdrThread(thread.id) ? <HerdrThreadBadge /> : null}
        ...
        {parentOptions && hasChildren ? (
          <SidebarChildToggleChevron
            ...
            expandLabel={t("sidebar.thread.expandThreads", {
              title: labelTitle,
            })}
            collapseLabel={t("sidebar.thread.collapseThreads", {
              title: labelTitle,
            })}
