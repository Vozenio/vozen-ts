...
export function PluginThreadList({
  ...
  const title =
    replacement.kind === "plugin"
      ? replacement.registration.title
      : t("sidebar.plugin.defaultTitle");

  const handleCrash = useCallback(
    (pluginId: string) => {
      toast.error(t("sidebar.plugin.crashedToastTitle"), {
        description: t("sidebar.plugin.crashedToastDescription", {
          title,
          pluginId,
        }),
      });
