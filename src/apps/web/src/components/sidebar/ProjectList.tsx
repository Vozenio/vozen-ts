...
function ProjectListProjectsSectionActions({
  ...
  return (
    <ProjectListSectionIconButton
      ariaLabel={t("sidebar.section.newProject")}
      title={t("sidebar.section.newProject")}
...
function ProjectListThreadsSectionActions({
  ...
  return (
    <>
      {onNewSection ? (
        <ProjectListSectionIconButton
          ariaLabel={t("sidebar.section.newSection")}
          title={t("sidebar.section.newSection")}
      ...
      <ProjectListSectionIconButton
        ariaLabel={t("sidebar.common.newThread")}
        title={t("sidebar.common.newThread")}
        icon={
          <Icon
...
const SIDEBAR_ORGANIZE_MODES = [
  "project",
  "machine",
  "chronological",
] as const satisfies readonly SidebarOrganizationMode[];

const SIDEBAR_SORT_MODES = [
  "updated",
  "created",
  "alpha",
] as const satisfies readonly SidebarChronologicalSort[];
...
export function SidebarDisplayOptionsMenu({
  ...
  const organizeModeLabels: Record<SidebarOrganizationMode, string> = {
    project: t("sidebar.section.byProject"),
    machine: t("sidebar.section.byMachine"),
    chronological: t("sidebar.section.manually"),
  };
  const sortModeLabels: Record<SidebarChronologicalSort, string> = {
    none: t("sidebar.section.updatedAt"),
    updated: t("sidebar.section.updatedAt"),
    created: t("sidebar.section.createdAt"),
    alpha: t("sidebar.section.alphabetical"),
  };

  return (
    <DropdownMenu open={open} onOpenChange={onOpenChange}>
      <SidebarDisplayMenuTrigger
        ariaLabel={t("sidebar.section.displayOptionsAria")}
        iconName="SlidersHorizontal"
        tooltip={t("sidebar.section.displayOptions")}
      />
      <DropdownMenuContent
        align="end"
        mobileTitle={t("sidebar.section.displayOptions")}
      >
        <DropdownMenuLabel className={CHROME_SECTION_LABEL_CLASS}>
          {t("sidebar.section.organize")}
        </DropdownMenuLabel>
        <DropdownMenuGroup aria-label={t("sidebar.section.organize")}>
          {SIDEBAR_ORGANIZE_MODES.map((mode) => (
            <DropdownMenuCheckboxItem
              key={mode}
            ...
            >
              {organizeModeLabels[mode]}
            </DropdownMenuCheckboxItem>
          ))}
        </DropdownMenuGroup>
        <DropdownMenuSeparator />
        <DropdownMenuLabel className={CHROME_SECTION_LABEL_CLASS}>
          {t("sidebar.section.sortBy")}
        </DropdownMenuLabel>
        <DropdownMenuGroup aria-label={t("sidebar.section.sortBy")}>
          {SIDEBAR_SORT_MODES.map((sort) => (
            <DropdownMenuCheckboxItem
              key={sort}
              checked={selectedSort === sort}
              onCheckedChange={() => setChronologicalSort(sort)}
            >
              {sortModeLabels[sort]}
            </DropdownMenuCheckboxItem>
          ))}
...
export function ProjectListNavigationLoadingState() {
  ...
  return (
    <div
      aria-label={t("sidebar.section.loadingNavigation")}
      ...
      {threadSearch?.isActive ? (
          ...
          <Button
            ...
            aria-label={
              threadSearch.query.trim()
                ? t("sidebar.search.clearAndClose")
                : t("sidebar.search.close")
            }
        ...
        <div className="flex min-w-0 items-center gap-0.5">
          <Button
            ...
            aria-label={
              newThreadShortcut
                ? t("sidebar.newThreadAction.withShortcut", {
                    shortcut: newThreadShortcut.label,
                  })
                : t("sidebar.common.newThread")
            }
            ...
            <span className="flex min-w-0 flex-1 items-center gap-1.5">
              ...
              {newThreadSplitIndicator.miniMap ? (
                <SplitPaneMiniMap
                  ...
                  label={t("sidebar.newThreadAction.openInSplit")}
                />
              ) : null}
              ...
              <Button
                ...
                aria-label={
                  threadSearchShortcut
                    ? t("sidebar.search.withShortcut", {
                        shortcut: threadSearchShortcut.label,
                      })
                    : t("sidebar.search.ariaLabel")
                }
...
function ProjectListComponent({
          ...
          onError: (error) =>
            setSectionCreateErrorMessage(
              getSectionMutationErrorMessage({
                error,
                fallbackMessage: t("sidebar.section.createFailed"),
                nameConflictMessage: t("sidebar.section.nameConflict"),
              }),
            ),
        },
      );
    },
    [createThreadSectionMutate, t],
  );
  ...
  const handleRenameThreadSection = useCallback(
        ...
        { id, name },
        {
          ...
          onError: (error) =>
            setSectionRenameErrorMessage(
              getSectionMutationErrorMessage({
                error,
                fallbackMessage: t("sidebar.section.renameFailed"),
                nameConflictMessage: t("sidebar.section.nameConflict"),
              }),
            ),
        },
    ...
    [sectionRenameDialog, t, updateThreadSectionMutate],
  );
  ...
  const sectionDeleteDialogContent = (
    ...
    >
      {sectionDeleteDialog.target ? (
        <ConfirmDeleteDialogContent
          title={t("sidebar.section.removeSectionTitle")}
          description={t("sidebar.section.removeSectionDescription")}
          confirmLabel={t("sidebar.section.removeSectionConfirm")}
