...
import { getRootComposeRoutePath, getSettingsRoutePath, getThreadRoutePath } from "@/lib/route-paths";
...
export function AppSidebar({
  ...
  const newThreadSplit = usePaneContentSplitDrag({
    ...
    enabled: true,
    label: t("sidebar.common.newThread"),
  });
        ...
        <SidebarMenu className="flex-row flex-wrap-reverse items-center gap-1">
          <SidebarMenuItem className="min-w-0">
            <SidebarMenuButton
              ...
              aria-label={
                settingsShortcut
                  ? t("sidebar.footer.settingsWithShortcut", {
            ...
            >
              <Link to={settingsRoutePath} onClick={closeOnMobile}>
                <Icon name="Settings" />
                <span className="sr-only">{t("sidebar.footer.settings")}</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
          ...
          <SidebarMenuItem className="min-w-0">
            <SidebarMenuButton
              className={SIDEBAR_FOOTER_ACTION_CLASS}
              tooltip={{
                children: t("sidebar.footer.remoteAccess"),
                hidden: false,
                side: "top",
              }}
              aria-label={t("sidebar.footer.remoteAccess")}
              onClick={() => {
                closeOnMobile();
                navigate(getSettingsRoutePath("remote-access"));
              }}
            >
              <Icon name="Smartphone" />
              <span className="sr-only">
                {t("sidebar.footer.remoteAccess")}
              </span>
            </SidebarMenuButton>
          </SidebarMenuItem>
