...
export function SidebarUpdatesBadge({ onNavigate }: SidebarUpdatesBadgeProps) {
  ...
  const bbLabel =
    bbUpdateCount === 1
      ? t("sidebar.updates.bbUpdateAvailable")
      : t("sidebar.updates.bbUpdatesAvailable");
  const providerLabel = t("sidebar.updates.providerAvailable", {
    names: joinNames(staleProviders.map((stale) => stale.displayName)),
    word:
      staleProviders.length === 1
        ? t("sidebar.updates.update")
        : t("sidebar.updates.updates"),
  });
