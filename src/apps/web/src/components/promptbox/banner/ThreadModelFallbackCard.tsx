...
export function ThreadModelFallbackCard({
  ...
  return (
    <PromptStackCard
      ariaLabel={t("promptbox.threadModelFallbackCard.ariaLabel")}
    ...
    >
      ...
      >
        ...
        <span className="shrink-0 font-medium text-foreground">
          {t("promptbox.threadModelFallbackCard.ariaLabel")}
        </span>
        <span
          className="min-w-0 flex-1 truncate text-muted-foreground"
          title={fallback.message}
        >
          {t("promptbox.threadModelFallbackCard.switchedFromTo", {
            fromModel: modelLabel(fallback.originalModel),
            toModel: modelLabel(fallback.fallbackModel),
          })}
        </span>
        <button
          type="button"
          aria-label={t("promptbox.threadModelFallbackCard.dismiss")}
