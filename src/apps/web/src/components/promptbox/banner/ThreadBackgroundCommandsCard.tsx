...
function backgroundActivityDisplay(
  ...
  if (isBackgroundAgentTaskType(row.taskType)) {
    return {
      icon: "UserRoundPlus",
      label: t("promptbox.threadBackgroundCommandsCard.agentLabel"),
      runningPrefix: t(
        "promptbox.threadBackgroundCommandsCard.agentRunningPrefix",
      ),
    };
  }
  return {
    icon: "Terminal",
    label: t("promptbox.threadBackgroundCommandsCard.commandLabel"),
    runningPrefix: t(
      "promptbox.threadBackgroundCommandsCard.commandRunningPrefix",
    ),
  };
}
...
function backgroundActivityGroupLabel(
  ...
  if (hasAgent && hasCommand) {
    return t("promptbox.threadBackgroundCommandsCard.groupLabelBoth");
  }
  return hasAgent
    ? t("promptbox.threadBackgroundCommandsCard.groupLabelAgents")
    : t("promptbox.threadBackgroundCommandsCard.groupLabelCommands");
}
...
function backgroundActivityAriaLabel(
  t: TFunction,
  row: TimelineWorkflowWorkRow,
  label = backgroundActivityDisplay(t, row).label,
): string {
  const model = backgroundActivityModel(row);
  return model
    ? t("promptbox.threadBackgroundCommandsCard.ariaLabelWithModel", {
        label,
        description: row.description,
        model,
      })
    : t("promptbox.threadBackgroundCommandsCard.ariaLabelNoModel", {
        label,
        description: row.description,
      });
}
...
function compactBackgroundActivityLabel(
  ...
  if (commandCount === 0) {
    return t("promptbox.threadBackgroundCommandsCard.compactAgentsOnly", {
      count: agentCount,
    });
  }
  if (agentCount === 0) {
    return t("promptbox.threadBackgroundCommandsCard.compactCommandsOnly", {
      count: commandCount,
    });
  }
  return t("promptbox.threadBackgroundCommandsCard.compactMixed", {
    count: rows.length,
  });
}
...
function BackgroundActivitySummary({
  ...
  const display = backgroundActivityDisplay(t, row);
  const model = backgroundActivityModel(row);
  return (
    <span className="flex min-w-0 flex-1 items-center gap-1 text-left">
      ...
      {model ? (
        <span
          ...
          title={t("promptbox.threadBackgroundCommandsCard.modelTitle", {
            model,
          })}
        >
          {model}
...
export function ThreadBackgroundCommandsCard({
  ...
  const compactLabel = compactBackgroundActivityLabel(t, commands);
  const primaryDisplay = backgroundActivityDisplay(t, primary);
  const groupLabel = backgroundActivityGroupLabel(t, commands);
        ...
        {canExpand ? (
          <button
            ...
            aria-label={
              useCompactSummary
                ? compactLabel
                : backgroundActivityAriaLabel(t, primary, groupLabel)
            }
            onClick={onToggle}
            ...
            {useCompactSummary ? (
              ...
              <>
                ...
                <span className={activityMetaClass("active", "shrink-0")}>
                  {t("promptbox.threadBackgroundCommandsCard.moreCount", {
                    count: others.length,
                  })}
                </span>
              </>
          ...
          <div
            ...
            aria-label={backgroundActivityAriaLabel(t, primary)}
          >
            <Icon
              name={primaryDisplay.icon}
              className={activityIconClass("active", "size-3.5 shrink-0")}
                ...
                >
                  ...
                  {model ? (
                    <span
                      ...
                      title={t(
                        "promptbox.threadBackgroundCommandsCard.modelTitle",
                        { model },
                      )}
                    >
                      {model}
