...
export const ThreadEnvironmentSummary = memo(function ThreadEnvironmentSummary({
  ...
  return (
    <div className="flex min-w-0 max-w-full items-center gap-2 pr-1.5">
      {projectName ? (
        <OptionDisplay
          label={t("promptbox.threadEnvironmentSummary.projectLabel")}
          value={projectName}
          compactValue={projectName}
          leading={<Icon name="Folder" className="size-4 shrink-0" />}
          className="h-6 max-w-[10rem] shrink-0"
          title={t("promptbox.threadEnvironmentSummary.projectTitle", {
            name: projectName,
          })}
          muted
        />
      ...
      <OptionDisplay
        label={t("promptbox.threadEnvironmentSummary.environmentLabel")}
        ...
        className="h-6 max-w-[10rem] shrink-0"
        title={t("promptbox.threadEnvironmentSummary.environmentTitle", {
          ...
          onClick={() => {
            void copyToClipboardWithToast(checkoutCopyValue, {
              successMessage:
                ...
                t("promptbox.threadEnvironmentSummary.valueCopied"),
              errorMessage:
                environmentCheckout.copyErrorMessage ??
                t("promptbox.threadEnvironmentSummary.copyFailed"),
            });
          }}
        >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              aria-label={t(
                "promptbox.threadEnvironmentSummary.createNewThreadInWorktree",
              )}
