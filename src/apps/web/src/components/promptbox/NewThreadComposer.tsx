...
export function resolveNewThreadSubmitDisabledReason({
  ...
  // Not a hook: this is called from plain data-transform code, and is also
  // unit-tested standalone (RootComposeView.test.ts) with a fixed single-arg
  // signature. Falls back to the English copy when i18next has not been
  // initialized yet (e.g. in that test).
  const t = (key: string, fallback: string): string =>
    i18next.t(key) ?? fallback;
  if (isSubmitting) {
    return t("promptbox.newThreadComposer.startingThread", "Starting thread...");
  }
  if (isCopyingAttachments) {
    return t(
      "promptbox.newThreadComposer.movingAttachments",
      "Moving attachments to the selected project...",
    );
  }
  if (isUploading) {
    return t(
      "promptbox.newThreadComposer.uploadingAttachments",
  ...
  if (projectDefaultsUnavailable) {
    return projectDefaultsStatus === "error"
      ? t(
          "promptbox.newThreadComposer.couldNotLoadProjectDefaults",
          "Could not load the project's execution defaults.",
        )
      : t(
          "promptbox.newThreadComposer.loadingProjectDefaults",
          "Loading the project's execution defaults...",
        );
  }
  if (!selectedProviderId) {
    return t("promptbox.newThreadComposer.selectProvider", "Select a provider.");
  }
  if (isLoadingModels) {
    return t(
      "promptbox.newThreadComposer.loadingModels",
  ...
  if (!selectedThreadModel) {
    return t("promptbox.newThreadComposer.selectModel", "Select a model.");
  }
  if (submissionEnvironmentUnavailable) {
    return t(
      "promptbox.newThreadComposer.selectEnvironment",
  ...
  if (promptInputEmpty) {
    return t(
      "promptbox.newThreadComposer.enterPromptOrAttachFile",
...
export function NewThreadComposer({
  ...
  const handleAttachFiles = useCallback(
            ...
            if (currentUploadTargetRef.current === capturedTarget) {
              setAttachmentError(
                getMutationErrorMessage({
                  error,
                  fallbackMessage: t(
                    "promptbox.newThreadComposer.attachmentUploadFailed",
                  ),
                }),
              );
            }
    ...
    [projectId, promptDraft, t, uploadPromptAttachment],
  );
          ...
          try {
            ...
            setAttachmentError(
              getMutationErrorMessage({
                error,
                fallbackMessage: t(
                  "promptbox.newThreadComposer.attachmentsMoveFailed",
                ),
              }),
            );
            return;
