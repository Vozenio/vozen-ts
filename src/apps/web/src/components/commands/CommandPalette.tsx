...
export function CommandPalette({ threadId, projectId }: CommandPaletteProps) {
  ...
  const palettePlaceholder = t("commands.palette.searchPlaceholder");
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div
          ...
          id={listId}
          role="listbox"
          aria-label={t("commands.palette.listLabel")}
          className="max-h-[min(24rem,50dvh)] overflow-y-auto p-1"
        >
          {ranked.length === 0 ? (
            <p className="px-2 py-6 text-center text-sm text-muted-foreground">
              {t("commands.palette.noResults")}
            </p>
          ) : (
