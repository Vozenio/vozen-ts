...
export function BrowserFindBar({
  ...
  return (
    <div
      ...
      role="search"
      aria-label={t("secondaryPanel.browserFindBar.findInPage", "Find in page")}
      className={cn(
    ...
    >
      <div className="flex h-7 min-w-0 flex-1 items-center gap-2 rounded-md border border-border/70 bg-background/70 px-2">
        <input
          ...
          placeholder={t("secondaryPanel.browserFindBar.findInPage", "Find in page")}
          aria-label={
            shortcut
              ? t(
                  "secondaryPanel.browserFindBar.findInPageWithShortcut",
                  "Find in page ({{shortcut}})",
                  { shortcut: shortcut.label },
                )
              : t("secondaryPanel.browserFindBar.findInPage", "Find in page")
          }
      ...
      <FindBarButton
        icon="ChevronUp"
        label={t("secondaryPanel.browserFindBar.previousMatch", "Previous match")}
        disabled={!hasMatches}
        onClick={onFindPrevious}
      />
      <FindBarButton
        icon="ChevronDown"
        label={t("secondaryPanel.browserFindBar.nextMatch", "Next match")}
        disabled={!hasMatches}
        onClick={onFindNext}
      />
      <FindBarButton
        icon="X"
        label={t("secondaryPanel.browserFindBar.closeFindBar", "Close find bar")}
        onClick={onClose}
      />
    </div>
  );
