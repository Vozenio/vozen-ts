...
export function ThreadSecondaryPanel({
  ...
  const hidePanelLabel = t(
    "secondaryPanel.threadSecondaryPanel.hidePanel",
    "Hide right panel",
  );
        ...
        >
          ...
          >
            <div
              ...
              role="toolbar"
              aria-label={t(
                "secondaryPanel.threadSecondaryPanel.rightPanelViews",
                "Right panel views",
              )}
            >
              {renderPanelTabGroup({
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-sidebar">
          ...
          {isBrowserSurfaceActive ? null : hasActiveSurfaceTab ? (
            ...
            >
              {surfaceContent ?? (
                <EmptyStatePanel className="mx-4 rounded-lg">
                  {t(
                    "secondaryPanel.threadSecondaryPanel.noFilePreviewContent",
            ...
            <EmptyStatePanel className="m-4 rounded-lg" role="status">
              {resolvedGitDiffTabStatus === "error" ? (
                <div className="flex flex-col items-center gap-3 text-center">
                  <span>
                    {t(
                      "secondaryPanel.threadSecondaryPanel.gitDetectionError",
                  ...
                  {onRetryGitDiffEligibility ? (
                    ...
                    >
                      {t("secondaryPanel.threadSecondaryPanel.retry", "Retry")}
                    </Button>
                  ) : null}
                </div>
              ) : (
                t(
                  "secondaryPanel.threadSecondaryPanel.checkingGitSupport",
            ...
            <EmptyStatePanel className="m-4 rounded-lg">
              {t(
                "secondaryPanel.threadSecondaryPanel.panelViewUnavailable",
...
function NewTabButton({
  ...
  const { t } = useTranslation();
  const openNewTabLabel = t(
    "secondaryPanel.threadSecondaryPanel.openNewTab",
    "Open new tab",
  );
  return (
    <Button
      ...
      aria-label={
        shortcut ? `${openNewTabLabel} (${shortcut.label})` : openNewTabLabel
      }
...
function SecondaryPanelResizeHandle({
  ...
  return (
    <PanelResizeHandle
      ...
      aria-label={t(
        "secondaryPanel.threadSecondaryPanel.resizeThreadAndRightPanel",
