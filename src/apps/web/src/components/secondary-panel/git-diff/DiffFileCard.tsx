...
function DiffFileCardLoadDiffNotice({
  ...
  return (
    <div className={DIFF_FILE_CARD_NOTICE_CLASS}>
      <span>
        {entry.binary
          ? t("secondaryPanel.gitDiff.diffFileCard.binaryFile", "Binary file.")
          : t(
              "secondaryPanel.gitDiff.diffFileCard.changedLinesCount",
              "{{changedLines}} changed lines.",
              { changedLines: changedLines.toLocaleString() },
            )}
      </span>
      <Button
        type="button"
        ...
        onClick={onLoadPatch}
      >
        {t("secondaryPanel.gitDiff.diffFileCard.loadDiff", "Load diff")}
      </Button>
    </div>
  );
}
    ...
    return (
      <div className={DIFF_FILE_CARD_NOTICE_CLASS}>
        <span className="text-destructive">
          {patchState.error ??
            t(
              "secondaryPanel.gitDiff.diffFileCard.loadError",
        ...
        <Button
          ...
          onClick={onRetry}
        >
          {t("secondaryPanel.gitDiff.diffFileCard.retry", "Retry")}
        </Button>
      </div>
    );
  }
  ...
  if (entry.loadMode === "too_large") {
    return (
      <div className={DIFF_FILE_CARD_NOTICE_CLASS}>
        <span>
          {t(
            "secondaryPanel.gitDiff.diffFileCard.tooLarge",
        ...
        {onOpenFilePreview ? (
          <FilePathLink
            path={entry.path}
            displayName={t(
              "secondaryPanel.gitDiff.diffFileCard.openFile",
              "Open file",
            )}
    ...
    if (patchState.status === "loaded") {
      return (
        <div className={DIFF_FILE_CARD_NOTICE_CLASS}>
          <span>
            {t(
              "secondaryPanel.gitDiff.diffFileCard.noRenderableDiff",
    ...
    <>
      ...
      {truncated ? (
        <div className={DIFF_FILE_CARD_NOTICE_CLASS}>
          <span>
            {t(
              "secondaryPanel.gitDiff.diffFileCard.truncated",
          ...
          {onOpenFilePreview ? (
            <FilePathLink
              path={entry.path}
              displayName={t(
                "secondaryPanel.gitDiff.diffFileCard.showFullDiff",
                "Show full diff",
              )}
              onClick={() => onOpenFilePreview(entry.path)}
              className="text-xs underline underline-offset-4"
