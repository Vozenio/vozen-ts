  ...
  },
  {
    ...
    commands: [
      paletteHiddenCommand(
        "palette.open",
        "Open quick palette",
        "Search and run vozen commands from the keyboard.",
      ),
      command("window.new", "New window", "Open another vozen desktop window."),
      command("settings.open", "Open settings", "Open vozen settings."),
      command(
        "settings.openServers",
        "Open server settings",
        "Open settings to add or manage vozen servers.",
      ),
      command(
        "sidebar.toggle",
        "Toggle sidebar",
      ...
      command(
        ...
        "Open the desktop log viewer for the vozen server and host daemon.",
      ),
    ],
  },
  {
  ...
  },
  {
    ...
    commands: QUESTION_SELECT_APP_COMMAND_IDS.map((id, index) =>
      paletteHiddenCommand(
        ...
        `Choose visible answer ${index + 1} when vozen asks a question.`,
      ),
    ),
  },
];
