...
export function ProjectSettingsView() {
  ...
  const addSourceButtons = multipleMachines ? (
    <div className="mt-2 flex gap-2">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          ...
          >
            {t("views.projectSettings.addOnAMachine")}
          </Button>
        </DropdownMenuTrigger>
              ...
              >
                ...
                {hasSource ? (
                  <span className="shrink-0 text-xs text-subtle-foreground">
                    {t("views.projectSettings.alreadyAdded")}
                  </span>
                ) : !connected ? (
                  <span className="shrink-0 text-xs text-subtle-foreground">
                    {t("views.projectSettings.offline")}
                  </span>
                ) : null}
              </DropdownMenuItem>
            );
    ...
    <div className="mt-2 flex gap-2">
      ...
      >
        {t("views.projectSettings.addLocalPath")}
      </Button>
    </div>
  ) : null;
  ...
  return (
    <PageShell contentClassName="pt-4 md:pt-5">
      <div className="mx-auto w-full max-w-3xl space-y-6">
        <SettingsSection title={t("views.projectSettings.title")}>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">
              {t("views.projectSettings.loading")}
            </p>
          ) : sources.length === 0 ? (
            <div>
              <p className="text-sm text-muted-foreground">
                {t("views.projectSettings.noSourcesConfigured")}
              </p>
              {addSourceButtons}
            </div>
          ) : (
