...
function getPullRequestMergeLoadingTitle(
  ...
  switch (method) {
    case "merge":
      return i18next.t("views.threadDetail.view.pullRequest.mergeMerging");
    case "squash":
      return i18next.t("views.threadDetail.view.pullRequest.mergeSquashing");
    case "rebase":
      return i18next.t("views.threadDetail.view.pullRequest.mergeRebasing");
  }
}
...
type ThreadDetailViewProps =
  ThreadDetailViewPageProps | ThreadDetailViewPaneProps;
...
function buildHostConnectionNotice(
  ...
  const subject =
    hostName ?? i18next.t("views.threadDetail.view.hostConnection.host");
  return {
    label:
      displayStatus === "host-reconnecting"
        ? i18next.t(
            "views.threadDetail.view.hostConnection.disconnectedReconnecting",
            { subject },
          )
        : i18next.t("views.threadDetail.view.hostConnection.disconnected", {
            subject,
          }),
...
function buildOpenTargetMenuItemLabel(target: WorkspaceOpenTarget): string {
  return i18next.t("views.threadDetail.view.localFile.openTargetLabel", {
    label: target.label,
  });
}
  ...
  return (
    <PageShell contentClassName="min-h-full items-center justify-center">
      <p className="py-12 text-center text-sm text-destructive">
        {t("views.threadDetail.view.common.notFound")}
      </p>
    </PageShell>
  );
}
...
function ThreadDetailViewInternal(props: ThreadRoutePathArgs) {
  ...
  const updateThread = useUpdateThread({
    errorMessage: t("views.threadDetail.view.thread.assignParentFailed"),
  });
  ...
  useEffect(() => {
    ...
    appToast.warning(t("views.threadDetail.view.thread.editSessionExpired"));
        ...
        .catch((error) => {
          appToast.error(
            getMutationErrorMessage({
              error,
              fallbackMessage: t(
                "views.threadDetail.view.thread.editMessageFailed",
              ),
    ...
    () =>
      threadFixedViewTabs.map((tab) =>
        tab.kind === "thread-info"
          ? {
              ariaLabel: t(
                "views.threadDetail.view.secondaryPanel.threadInfoAriaLabel",
              ),
              label: t(
                "views.threadDetail.view.secondaryPanel.threadInfoLabel",
              ),
              leadingVisual: <Icon name="Info" />,
              onSelect: () => handleSecondaryPanelChange("thread-info"),
              tab,
              title: t(
                "views.threadDetail.view.secondaryPanel.threadInfoTitle",
              ),
            }
          : {
              ariaLabel: t(
                "views.threadDetail.view.secondaryPanel.diffAriaLabel",
              ),
              label: t("views.threadDetail.view.secondaryPanel.diffLabel"),
              leadingVisual: <Icon name="FileDiff" />,
              onSelect: () => handleSecondaryPanelChange("git-diff"),
              tab,
              title: t("views.threadDetail.view.secondaryPanel.diffTitle"),
            },
      ),
    [handleSecondaryPanelChange, threadFixedViewTabs, t],
  );
  ...
  const handlePullRequestReady = useCallback(async () => {
    ...
    const toastId = appToast.loading(
      t("views.threadDetail.view.pullRequest.markingReady"),
    );
    try {
      ...
      if (response.action !== "pull_request_ready") {
        throw new Error(
          t("views.threadDetail.view.pullRequest.readyResponseError"),
        );
      }
      appToast.success(response.message, { id: toastId });
    } catch (error) {
      appToast.error(
        t("views.threadDetail.view.pullRequest.updateFailedTitle"),
        {
          id: toastId,
          description: getMutationErrorMessage({
            error,
            fallbackMessage: t(
              "views.threadDetail.view.pullRequest.updateFailedFallback",
            ),
          }),
        },
      );
    }
  }, [requestEnvironmentAction, t, thread?.environmentId]);
  const handlePullRequestDraft = useCallback(async () => {
    ...
    const toastId = appToast.loading(
      t("views.threadDetail.view.pullRequest.convertingToDraft"),
    );
    try {
      const response = await requestEnvironmentAction.mutateAsync({
        id: environmentId,
      ...
      if (response.action !== "pull_request_draft") {
        throw new Error(
          t("views.threadDetail.view.pullRequest.draftResponseError"),
        );
      }
      appToast.success(response.message, { id: toastId });
    } catch (error) {
    ...
    async (method: PullRequestMergeMethod) => {
      ...
      try {
        ...
        if (response.action !== "pull_request_merge") {
          throw new Error(
            t("views.threadDetail.view.pullRequest.mergeResponseError"),
          );
        }
        appToast.success(response.message, { id: toastId });
      } catch (error) {
        appToast.error(
          t("views.threadDetail.view.pullRequest.mergeFailedTitle"),
          {
            id: toastId,
            description: getMutationErrorMessage({
              error,
              fallbackMessage: t(
                "views.threadDetail.view.pullRequest.mergeFailedFallback",
              ),
            }),
          },
        );
  ...
  const handleTimelineLocalFileLinkResolution = useCallback(
    (
      ...
      if (resolution.kind === "error") {
        appToast.error(t("views.threadDetail.view.localFile.failedToOpen"), {
    ...
    [openHostFile, openStorageFile, openWorkspaceFile, t],
  );
      ...
      void refetchThreadStorageFiles()
        .then((result) => {
          ...
          if (resolvedThreadStorageRootPath === null) {
            appToast.error(
              t("views.threadDetail.view.localFile.failedToOpen"),
              {
                description: t(
                  "views.threadDetail.view.localFile.storagePathUnavailable",
                ),
              },
            );
            return;
    ...
    (link: ThreadTimelineLocalFileLink) => {
      ...
      if (openTargetItems.length > 0) {
        items.push({
          ...
          label: t("views.threadDetail.view.localFile.contextMenu.openIn"),
          type: "submenu",
        });
      }
        ...
        items.push(
          {
            id: "builtin",
            label: t(
              "views.threadDetail.view.localFile.contextMenu.openWithBuiltInPreview",
            ),
            onSelect: () => {
          ...
          ...matching.map((opener) => ({
            ...
            label: t(
              "views.threadDetail.view.localFile.contextMenu.openWithPlugin",
              { title: opener.title },
            ),
            onSelect: () => {
      ...
      items.push(
        {
          id: "copy-path",
          label: t(
            "views.threadDetail.view.localFile.contextMenu.copyFilePath",
          ),
          onSelect: () => {
            void copyToClipboardWithToast(link.path, {
              successMessage: t(
                "views.threadDetail.view.localFile.contextMenu.copyFilePathSuccess",
              ),
              errorMessage: t(
                "views.threadDetail.view.localFile.contextMenu.copyFilePathError",
              ),
            });
          },
        },
        {
          id: "copy-name",
          label: t(
            "views.threadDetail.view.localFile.contextMenu.copyFileName",
          ),
          onSelect: () => {
            void copyToClipboardWithToast(getLocalFileBasename(link.path), {
              successMessage: t(
                "views.threadDetail.view.localFile.contextMenu.copyFileNameSuccess",
              ),
              errorMessage: t(
                "views.threadDetail.view.localFile.contextMenu.copyFileNameError",
              ),
            });
          },
        },
  ...
  if (!thread || thread.projectId !== projectId) {
    return (
      <PageShell contentClassName="min-h-full items-center justify-center">
        <p className="py-12 text-center text-sm text-destructive">
          {error
            ? t("views.threadDetail.view.error.failedToLoadThread")
            : t("views.threadDetail.view.common.notFound")}
        </p>
      </PageShell>
    );
  ...
  const responsiveWorkspaceActions: ThreadActionsMenuResponsiveAction[] =
    workspaceOpenPath && preferredDirectoryTarget
      ? [
          ...
          label: t("views.threadDetail.view.workspace.openInTargetLabel", {
            label: target.label,
          }),
          onSelect: async () => {
      ...
      switch (tab.kind) {
        case "browser": {
          ...
          return {
            ...shared,
            label:
              browserLabel.length > 0
                ? browserLabel
                : t("views.threadDetail.view.tabs.browserFallback"),
            leadingVisual: (
              <Icon
                name="Globe"
        ...
        case "terminal": {
          ...
          return {
            ...
            label:
              session?.title ??
              t("views.threadDetail.view.tabs.terminalFallback"),
            leadingVisual: (
              <Icon
                name="Terminal"
                className={COARSE_POINTER_COMPACT_ICON_SIZE_CLASS}
        ...
        case "new-tab":
          return {
            ...shared,
            label: t("views.threadDetail.view.tabs.newTab"),
            leadingVisual: (
              <Icon
                name="NewTab"
                className={COARSE_POINTER_COMPACT_ICON_SIZE_CLASS}
            ...
            timeline={{
              ...
              ongoingIndicatorLabel:
                thread.runtime.displayStatus === "host-reconnecting"
                  ? t(
                      "views.threadDetail.view.hostConnection.waitingForReconnection",
                    )
                  : undefined,
              timelineRows,
