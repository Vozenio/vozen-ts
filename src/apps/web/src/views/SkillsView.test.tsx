...
describe("SkillsOverview", () => {
  it("defaults to BB skills and places vozen Official skills first", () => {
    const markup = render({
      skills: [
    ...
    expect(markup).toContain('aria-label="Filters: Provider: vozen"');
    ...
    expect(markup).toContain("vozen Official");
    expect(markup).toContain("New vozen skill");
  ...
  it("labels the Type filter and preserves independent source toggles", async () => {
    ...
    for (const name of ["vozen Official", "Included in plugin", "User"]) {
      expect(
        screen
    ...
    expect(
      screen.getByLabelText(
        "automations is included with Automations (vozen plugin)",
      ).textContent,
    ).toBe("Included");
  ...
  it("puts every non-builtin, non-plugin scope in the User bucket", async () => {
    ...
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "vozen" }));
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "User" }));
  ...
  it("toggles vozen Official independently from Included in plugin", async () => {
    renderDom(
      <SkillsOverview
        providerRoster={NO_PROVIDER_ROSTER}
        skills={[
    ...
    fireEvent.click(
      screen.getByRole("menuitemcheckbox", { name: "vozen Official" }),
    );
    expect(await screen.findByText("official-skill")).toBeTruthy();
    expect(screen.getByText("automations")).toBeTruthy();
  ...
  it("lists a provider filter only for providers present in the skills", async () => {
    ...
    expect(
      screen
        .getByRole("menuitemcheckbox", { name: "vozen" })
        .getAttribute("aria-disabled"),
    ).toBeNull();
  });
  ...
  it("labels the Provider filter and prefixes its logo tooltip", async () => {
    ...
    expect(
      screen.getByRole("menuitemcheckbox", { name: "vozen" }).querySelector("img"),
    ).not.toBeNull();
  });
  ...
  it("keeps the default BB filter selected when only provider skills exist", async () => {
    ...
    fireEvent.pointerDown(screen.getByRole("button", { name: /^Filters/ }));
    const bbFilter = screen.getByRole("menuitemcheckbox", { name: "vozen" });
...
describe("SkillsLibrary registry detail lifecycle", () => {
  ...
  it("opens on Browse before Library and can start a skill from the registry", async () => {
    ...
    let forkButton = await screen.findByRole("button", {
      name: "Fork Useful skill into a new vozen skill",
    });
  ...
  it("renders the authoritative page order, exposes social proof, and loads more on scroll", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Fork Alpha into a new vozen skill",
      }).textContent,
    ).toBe("");
    const zuluCreate = screen.getByRole("button", {
      name: "Fork Zulu into a new vozen skill",
    });
...
describe("SkillDetailDialogView", () => {
  it("presents a built-in skill as vozen Official without an actions menu", async () => {
    ...
    const official = screen.getByLabelText("bb-cli is vozen Official");
    expect(official.textContent).toBe("vozen Official");
  ...
  it.each([
    ...
    },
    {
      ...
      accessibleLabel:
        "plugin-notes is included with Skill catalog fixture (vozen plugin)",
  ...
  it("uses a hoverable copy target and delegates editing to the thread flow", () => {
    ...
    expect(screen.queryByText("vozen Official", { exact: true })).toBeNull();
