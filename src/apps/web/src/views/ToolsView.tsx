...
function PluginDetailToolView({ pluginId }: { pluginId: string }) {
  ...
  const pluginToggle = useMutation({
    mutationFn: async (plugin: PluginListItem) => {
      ...
      try {
        ...
        throw new Error(t("views.tools.toggleError", { action }));
      }
    },
  ...
  const pluginDelete = useMutation({
    mutationFn: async (plugin: PluginListItem) => {
      try {
        ...
        throw new Error(t("views.tools.deleteError"));
      }
    },
    onSuccess: (_data, deletedPlugin) => {
      appToast.success(
        pluginIsLocalSource(deletedPlugin)
          ? t("views.tools.removedToast")
          : t("views.tools.uninstalledToast"),
      );
  ...
  if (listQuery.isError) {
    detailContent = (
      <ResourceListState
        state="error"
        message={t("views.tools.loadError")}
        layout="detail"
    ...
    detailContent = (
      <ResourceListState
        state="loading"
        message={t("views.tools.loadingPlugin")}
        layout="detail"
        maxWidthClassName="max-w-5xl"
      />
    );
    ...
    detailContent = (
      <ResourceListState
        state="error"
        message={t("views.tools.installedLoadError")}
        layout="detail"
        maxWidthClassName="max-w-5xl"
        onRetry={() => void listQuery.refetch()}
      />
    ...
    detailContent = (
      <ResourceListState
        state="empty"
        message={t("views.tools.notFound")}
        layout="detail"
        maxWidthClassName="max-w-5xl"
      />
    );
              ...
              <ConfirmDeleteDialogContent
                title={
                  pluginIsLocalSource(deleteTarget)
                    ? t("views.tools.confirmRemoveTitle")
                    : t("views.tools.confirmUninstallTitle")
                }
