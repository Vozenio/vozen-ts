...
describe("PluginDetail official catalog lifecycle", () => {
  ...
  it("explains why an incompatible official plugin cannot be installed", () => {
    ...
    const compatibilityStatus = screen
      .getByText("Update vozen to install this plugin")
  ...
  it("keeps catalog provenance and release management in the unified detail taxonomy", async () => {
    ...
    expect(screen.getByText("Updates with vozen")).toBeTruthy();
  ...
  it("shows a disabled Uninstall action for a built-in plugin", async () => {
    ...
    expect(
      await screen.findAllByText(
        "Included with vozen; disable this plugin instead.",
      ),
    ).not.toHaveLength(0);
  });
});
...
describe("plugin removal confirmation", () => {
  it("warns that removing a local plugin deletes its settings, secrets, and schedules and names the move path", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: "Remove from vozen" }),
    );

    expect(
      await screen.findByRole("heading", { name: "Remove plugin from vozen?" }),
    ).toBeTruthy();
    ...
    const description = screen.getByText(/Remove "github" from vozen/);
...
describe("PluginDetail runtime health", () => {
  ...
  it.each([
    [
      "incompatible",
      "This plugin version isn't compatible with your version of vozen.",
      "Update vozen to load a compatible bundled plugin.",
    ],
    [
      "missing",
      "The plugin's files are missing.",
      "Restart vozen. If the files are still missing, reinstall vozen.",
    ],
  ] as const)(
  ...
  it("keeps needs-configuration actionless because saving Settings retries it", () => {
    ...
    expect(alert.textContent).toContain(
      "Complete the Configuration section; vozen reloads the plugin after you save.",
    );
    expect(screen.queryByRole("button", { name: "Reload" })).toBeNull();
  });
