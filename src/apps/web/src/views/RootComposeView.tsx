...
export function RootComposeRightPanelToggle({
  ...
  const rightPanelLabel = isOpen
    ? t("views.rootCompose.view.rightPanelToggle.hide")
    : t("views.rootCompose.view.rightPanelToggle.show");
...
function RootComposeSurface({
        ...
        switch (tab.kind) {
            ...
            return {
              ...
              label:
                browserLabel.length > 0
                  ? browserLabel
                  : t("views.rootCompose.view.panelTabs.browserFallback"),
              leadingVisual: (
                <Icon
                  name="Globe"
          ...
          case "terminal": {
            ...
            return {
              ...
              label:
                session?.title ??
                t("views.rootCompose.view.panelTabs.terminalFallback"),
              leadingVisual: (
                <Icon
                  name="Terminal"
                  className={COARSE_POINTER_COMPACT_ICON_SIZE_CLASS}
          ...
          case "new-tab":
            return {
              ...shared,
              label: t("views.rootCompose.view.panelTabs.newTab"),
              leadingVisual: (
                <Icon
                  name="NewTab"
                  className={COARSE_POINTER_COMPACT_ICON_SIZE_CLASS}
    ...
    () => (
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pt-1">
        <EmptyStatePanel className="rounded-lg">
          {t("views.rootCompose.view.metadataPanel.noThreadDetails")}
        </EmptyStatePanel>
      </div>
    ),
    [t],
  ...
  const promptHeader = useMemo(() => {
    ...
    return (
      <div className="flex">
        ...
        <div
          aria-label={t("views.rootCompose.view.fork.forkingLabel", {
            title: forkSeed.sourceThreadTitle,
          })}
        ...
        >
          ...
          <span className="min-w-0 truncate">
            {t("views.rootCompose.view.fork.forkingLabel", {
              title: forkSeed.sourceThreadTitle,
            })}
          </span>
          <button
            type="button"
            aria-label={t("views.rootCompose.view.fork.cancelAriaLabel")}
  ...
  }, [forkSeed, handleCancelForkDraft, t]);
    ...
    return (
      <PageShell contentClassName="min-h-full items-center justify-center">
        <p className="py-12 text-center text-sm text-destructive">
          {t("views.rootCompose.view.errors.failedToLoadProjects")}
        </p>
      </PageShell>
    );
  }
  ...
  const promptBox = renderPromptBox({
    ...
    blockedReason: isProviderCliVersionBlocked
      ? t("views.rootCompose.view.providerCli.updateBeforeStarting", {
          provider: selectedProviderCliStatus?.displayName ?? selectedProviderId,
        })
      : undefined,
    resolveMentionLink,
    pluginComposerHost,
